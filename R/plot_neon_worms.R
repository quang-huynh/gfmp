...
# load example om from DLMtool
load("D:/GitHub/pbs-assess/gfmp/data/om.rda")
om@nsim <- 10
x <- runMSE(om, MPs = c("AvC", "CC1"))

# Psychedelic pyramid worms ---------------------------------------------------

plot_neon_worms <- function(object, type = c("SSB", "FM"), this_year = 2018,
  probs = c(0.2, 0.2)){

  ts <- gfdlm:::get_ts(object, type = c("SSB", "FM"), this_year = 2018)
  ...
  ggsave(file.path(fig_dir, "example-neon-worms.png"), width = 8, height = 6.6)
}
