...
We developed the R package [gfdlm](https://github.com/pbs-assess/gfdlm) [REF] to provide a set of tools that facilitate applications of this Framework.
The package includes our provisional library of MPs, a number of plotting functions for summarizing simulation output and model fits, and a variety of other utilities for manipulating DLMtool MPs and performance metrics, and creating reports based on DLMtool simulations (such as this document).
...
* `plot_factory()` is "the one [plotting function] to rule them all". This function takes a named list of DLMtool simulations along with a number of other arguments specifying characteristics like satisficed MPs and performance metrics and generates common versions of all the figures described below. The output is a named list of ggplot2 [REF] elements that can be viewed, modified, and/or saved for inclusion in a report. For example, this function was used to create all the main figures in the Rex Sole case study in this document (Figures \@ref(fig:rex-converge)--\@ref(fig:rex-proj-scenarios)).
...
Note that the `::` syntax refers to a function within the package gfdlm without loading the package via the `library()` function.
We write our code in this appendix using the syntax so that it is clear where various functions originate.
...
Note that `%>%` refers to the "pipe" function from the magrittr R package [REF].
...
This document was compiled with the R package csasdown [REF].
