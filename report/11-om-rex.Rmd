...
We fit the model with Stan [REF] using the model implemented in the gfplot R package [REF] according to the equation:
...
We used weakly informative priors: $k \sim \textrm{Normal}(0, 2)$, $L_\mathrm{inf} \sim \textrm{Normal}(0, \varphi)$, $t_0 \sim \textrm{Normal}(0, 20)$, and $\sigma \sim \textrm{Student-t}(3,0, 2)$, where $\varphi$ refers to the 99% quantile of the observed lengths (36 m).
We sampled 8000 iterations across four chains from the posterior and checked that the chains were consistent with convergence via the Rhat and ESS (effective sample size) metrics along with traceplots of the samples [REF].
...
We sampled 2000 replicates for each of four chains from the posterior and check that the chains were consistent with convergence using the same approach as described above for the von Bertalanffy model.
...
Our estimated average length at 50% maturity (`r round(mean(L50), 1)` cm) is between the length at 50% maturity reported for Rex Sole off Oregon (24cm) and the Gulf of Alaska (35cm) [@abookire2005].
...
```{r desc-stock-a-rex1, message=FALSE, warning=FALSE, echo=FALSE}
mlw <- gfplot::fit_length_weight(drex$survey_samples, sex = "all")
gfplot::plot_length_weight(object_all = mlw, col = c("All" = "black"))
```

```{r desc-stock-a-rex2, echo=TRUE, eval=FALSE}
mlw <- gfplot::fit_length_weight(drex$survey_samples, sex = "all")
gfplot::plot_length_weight(object_all = mlw, col = c("All" = "black"))
```

```{r desc-stock-a-rex, results = knitr_results, echo = knitr_echo, fig.cap="(ref:desc-stock-a-rex)", fig.width=6, out.width="4in", message=FALSE}
