...
mprex <- c(
  ...
  "Islope1", "Islope2", "Islope4", "IT10", "IT5", "Itarget1", "Lratio_BHI",
...
```{r, fig.cap="Historical trajectories from the Rex Sole WCVI operating model. Light shaded region represents the upper and lower bounds of the historical simulation. Thin grey lines represent 50 draws from the simulation.", fig.width=9.5, out.width="5in"}
...
```{r, fig.asp=1.2, out.width="4in", fig.width=6, fig.cap="Performance metric probability table across all MPs that were run for the Rex Sole WCVI closed-loop simulation. The MPs are ordered by performance metric from left to right and the shading corresponds to the probability value indicated in each cell. In future plots we retain only 'satisficed' MPs that have LT P40 > 0.70."}
...
```{r}
rex_satisficed <- dplyr::filter(rex_probs, `LT P40` > 0.70) %>%
  arrange(-`LT P40`) %>%
  pull(MP)
```
...
```{r, fig.asp=1.1, fig.cap="Timeseries projections for the Rex Sole WCVI closed-loop simulation. Solid blue line indicates median; darker and lighter shaded blue indicate 50 and 90 quantiles. Thin grey lines illustrate 3 sample iterations from the simulation. Horizontal dashed lines represent 1 and 0.8 and 0.4 in the case of B BMSY"}
gfdlm::plot_projection_ts(rex_mse_sub, type = c("F_FMSY", "B_BMSY"))
```


```{r, fig.cap="Kobe plots for the Rex Sole WCVI closed-loop simulation. Dots represent individual simulation iterations. B~BMSY~ light grey lines indicate 0.4 and 0.8 values. Contour lines indicate 0.10, 0.25 and 0.50 quantile kernel density estimates. Axes are cut off at maximum values of 4 although some simulation iterations extend beyond these boundaries.", fig.width=8, out.width="5.75in"}
gfdlm::plot_contours(rex_mse_sub, xlim = c(0, 4), ylim = c(0, 4), alpha = c(0.1, 0.25, 0.50))
```

```{r, fig.cap="Spider plots for the Rex Sole WCVI closed-loop simulation. Performance metrics are indicated at the end of each 'spoke'. Coloured lines represent the various management procedures.", out.width="4.5in", fig.width=10}
gfdlm::spider(rex_mse_sub, pm_list = PM, palette = "Set2", lwd = 1.0)
```
