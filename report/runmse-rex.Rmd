...
```{r, fig.asp=1.2, out.width="4in", fig.width=6, fig.cap="Performance metric probability table across all MPs that were run for the Rex Sole WCVI closed-loop simulation. The MPs are ordered by performance metric from left to right and the shading corresponds to the probability value indicated in each cell."}
...
```{r, fig.cap="Kobe plots for the Rex Sole WCVI closed-loop simulation. B~BMSY~ light grey lines indicate 0.4 and 0.8 values. Contour lines indicate 0.10, 0.25 and 0.50 quantile kernel density estimates."}
gfdlm::plot_contours(rex_mse_sub, xlim = c(0, 4), ylim = c(0, 4), alpha = c(0.1, 0.25, 0.50))
```

```{r, fig.cap="Spider plots for the Rex Sole WCVI closed-loop simulation.", out.width="4.5in", fig.width=10}
