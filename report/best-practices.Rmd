...
@punt2016 noted that care should be taken to avoid having too many performance metrics, which could make the final decision process too complex. They also advised that, to facilitate the decision process, performance metrics should be chosen so they can be understood by decision-makers and participants.

## Step 3. Selection of uncertainties/specification of OM(s)
...
Ideally, OMs should be calibrated to real data to ensure they can reproduce historical observations [e.g., @cox2008a; @forrest2018]. In very data-limited cases of course, this may not always be possible. In these cases, best practice would be to develop a set of OMs that differ in terms of key uncertainties, especially related to stock productivity and current depletion level.


## Step 4. Identification of candidate MPs
...
The role of science in this step is to ensure that results are clearly presented to decision-makers. Ideally this should include presentation of graphic outputs that enable clear comparison of MPs with respect to performance metrics and trade-offs [@punt2015]. For example, a key component of the DLMtool software is its graphic summaries [@carruthers2018]. The software also outputs a large range of result objects to enable analysts to create their own graphic and tabulated summaries.
...
Steps 2-5 of the best practice approach described above are implemented here using the software package DLMtool. We elected to use this software instead of other available software [e.g., @kell2007] for several reasons. The DLMtool software is very fast, making use of efficient coding practices and parallel processing. It also has more than 80 data-limited MPs programmed into it and has been subject to several previous peer-review processes, both at the institutional level (REFS) and in the scientific literature [e.g., @carruthers2014; @carruthers2016; @carruthers2018]. DLMtool is [open-source software](https://CRAN.R-project.org/package=DLMtool), enabling experienced users to customize MPs and performance metrics as needed. Finally, DFO has an ongoing partnership project with the developers of DLMtool, specifically aimed at customizing the software to meet Canadian policy needs.

*TODO: RF Get details of NOAA and CDFW peer reviews from Tom. Add some more recent dlmtool pubs to the bib and cite them here*

DLMtool has been designed to follow the best practices steps described above [@carruthers2018]. The software employs an object-oriented approach to construct the OMs and manage inputs and outputs of the MSE. Objects map directly onto the best practice steps described above, illustrated as boxes in Figure \@ref(fig:fig-dlmtool). Each object (box) contains numerous "slots", which keep track of the many inputs and outputs of the MSE. 

The OM object (Figure \@ref(fig:fig-dlmtool)) is constructed of four subcomponents: i) the implementation error model (Imp); the stock dynamics model (Stock); the fleet dynamics model (Fleet); and the observation model (Obs). The slots within these objects are parameters that may be derived from stock assessment models (e.g., estimates of natural mortality, unfished biomass, gear selectivity), data analyis (e.g., growth and maturity parameters), or expert judgement (e.g., form of the stock-recruitment relationship).

The Data object (Figure \@ref(fig:fig-dlmtool)) contains all of the data needed for implementing the MPs. It may be populated with simulated data from the OM during the MSE simulations, or with real data once the best MP has been selected.

The MP object (Figure \@ref(fig:fig-dlmtool)) contains all the information about the MPs used in the MSE, including the advice generated by the MPs. The Performance Metrics (PM) object (Figure \@ref(fig:fig-dlmtool)) keeps track all of the performance metrics for all the replicates under each MP.

Finally, the MSE object (Figure \@ref(fig:fig-dlmtool)) tracks all the outputs of the OM during the MSE process, including trajectories of TACs, catches, biomass, fishing mortality, stock status, reference points etc.

*TODO: Suggest we finish the section below once we have settled on a near final set of results. Possibly following meetings with GMU and the tech working group. Note RF has deleted bullets addressed in the text above (e.g., define satisficing)*
...
* Identify management procedure options
    - summarize candidates that we have narrowed it down to -- describe screening process
    - reference the detailed appendix

* Define a set of operating models that capture main uncertainties
    - Describe two options for building OMs - SRA vs What-if
    _ How did we evaluate plausibility of OMs?
...
* Simulate and compute performance indicators
       - how many replicates: checking for convergence of MP selection
       
*TODO: decide on terminology for iterations. Iterations? Trials? Replicates? Make sure we are consistent throughout*
