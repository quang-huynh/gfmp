...
@punt2016 note that care should be taken to avoid having too many performance metrics, which could make the final decision process too complex. They also advised that, to facilitate the decision process, performance metrics should be chosen so they can be understood by decision-makers and participants.
...
Steps 2--5 of the best practice approach described above are implemented here using the software package DLMtool. We elected to use this software instead of other available software [e.g., @kell2007] for several reasons. The DLMtool software is fast, making use of efficient coding practices and parallel processing. It also has more than 80 data-limited MPs programmed into it and has been subject to several previous peer-review processes, both at the institutional level (REFS) and in the scientific literature [e.g., @carruthers2014; @carruthers2016; @carruthers2018]. DLMtool is [open-source software](https://CRAN.R-project.org/package=DLMtool), enabling experienced users to customize MPs and performance metrics as needed. Finally, DFO has an ongoing partnership project with the developers of DLMtool, specifically aimed at customizing the software to meet Canadian policy needs.
