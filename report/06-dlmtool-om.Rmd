...
2^{-\frac{(L_{a} - L^{\textrm{FS}})^2}{\sigma^2_{\textrm{des}}}} & \textrm{if dome-shaped and } L_{a} \ge L^{\textrm{FS}}
\end{cases}
(\#eq:dlm-om-va)
$$
...
where $L_\textrm{rmax}$ is the length at maximum retention.
The standard deviation parameters of the ascending limb is given by the length at 5% retention $r_5$:
...
While the standard deviation of the descending limb is given by retention $r_{L_\infty}$ of fish of length $L_\infty$:
...
The retention curve approaches an asymptotic curve as $r_{L_\infty} \to 1$.
...
<!-- TODO: Check this is correct, see RF's correspondence with QH-->
