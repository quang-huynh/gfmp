...
Following the lead of @carruthers2018, we denote parameters that are user-defined and sampled stochastically across simulations with a tilde (e.g. $\widetilde{M}$).
...
<!-- TODO: check these! -->
$$
\theta_{a50} = \frac{- \ln\left( 1 - \widetilde{\theta}_{l50} / \widetilde{L}_{\infty} \right)}{\widetilde{\kappa}} + t_{0},
(\#eq:dlm-om-invonb)
$$
...
$\alpha^{\textrm{B}} = \frac{4\tilde{h}}{(1-\tilde{h})\phi_f^0}$,
$\beta^{\textrm{B}} = \frac{5\tilde{h}-1}{(1-\tilde{h})B_0}$
and
$\alpha^{\textrm{R}} = \frac{(5\tilde{h})^{1.25}}{\phi_f^0}$,
...
$$
\log \varepsilon_{R,y} = {\widetilde{\theta}}_{\textrm{AC}}\log \varepsilon_{R,y - 1} + \varepsilon_{R,y}\sqrt{\left( 1 - {{\widetilde{\theta}}_{\textrm{AC}}}^{2} \right)},
(\#eq:dlm-om-auto)
$$
...
v_{a} =
\begin{cases}
2^{-\frac{(\widetilde{L}_{a} - \widetilde{L}^{\textrm{FS}})^2}{\widetilde{\sigma}^2_{\textrm{asc}}}} & \textrm{if } L_{a} < L^{\textrm{FS}},\\
1 & \textrm{if logistic and } L_{a} \ge L^{\textrm{FS}},\\
2^{-\frac{(\widetilde{L}_{a} - \widetilde{L}^{\textrm{FS}})^2}{\widetilde{\sigma}^2_{\textrm{des}}}} & \textrm{if dome-shaped and } L_{a} \ge L^{\textrm{FS}}
\end{cases}
(\#eq:dlm-om-va)
$$

where $\widetilde{L}^\textrm{FS}$ represents the user-defined length-at-full-selectivity.
The standard deviation of the ascending limb ($\widetilde{\sigma}_{\textrm{sasc}}^{2}$) is given by

$$
\sigma_{\textrm{sasc}} = \frac{{\widetilde{L}}_{5} - \widetilde{L}^\textrm{FS}}{\sqrt{- \log_{2^{0.05}}}}
(\#eq:dlm-om-siglasc)
$$

where $\widetilde{L}_5$ is the user-defined length-at-5%-selectivity

The standard deviation of the descending limb is given by

$$
\sigma_{\textrm{sdesc}} = \frac{\widetilde{L}_\infty - \widetilde{L}^\textrm{FS}}{{\sqrt{- \log_{2^{\widetilde{V}_{L_\infty}}}}}},
(\#eq:dlm-om-sigldes)
$$
...
Fleet-specific selectivity in the SRA is calculated using Equation \@ref(eq:sra-om-vaf) with fleet-specific user-defined settings for $\widetilde{L}_5$, $\widetilde{L}_{FS}$ and $\widetilde{V}_{L_\infty}$, or is estimated if age- or length-composition data are provided.
...
The standard deviation parameters of the ascending limb is given by the length at 5% retention $\widetilde{L}_{r5}$:
...
and where $\widetilde{\beta}$ is the hyperstability/hyperdepletion parameter.
...
<!-- TODO: Shouldn't the epsiloni equation use SD instead of variance?? RF changed to sd-->
...
<!--TODO: Check this. RF cleaned it up, it was very hard to follow and I think some terms were mixed up in the text. Removed summation over r ... assume this was areas -->
...
<!-- Delete this
$$
C_{y,a}^{\textrm{obs}}\sim \textrm{Multinomial}\left( \textrm{ESS}_{\textrm{CAA}},p_{a} = \ \sum_{r}^{n_{r}}C_{y,a} \right)
(\#eq:dlm-om-cataobs)
$$
-->

$$
C_{y,a}^{\textrm{obs}}\sim \textrm{Multinomial}\left( \widetilde{\textrm{ESS}}_{\textrm{CAA}},p_{a} = \ C_{y,a} \right)
(\#eq:dlm-om-cataobs)
$$
...
$$
C_{y,a}^{\textrm{obs}} = \textrm{nint}\left( \ \frac{{{C}}_{y,a}^{\textrm{obs}}\ \widetilde{n}_{\textrm{CAA}}}{\widetilde{\textrm{ESS}}_{\textrm{CAA}}} \right).
...
The TAC implementation uncertainty term ($I_\textrm{TAC}$) is the product of a user-defined fraction of the TAC taken $b_\textrm{TAC}$ and a degree of inter-annual variability controlled by $\sigma_\textrm{TAC}$.
...
The error term $\varepsilon_{\textrm{TAC},y}$ is drawn from a normal distribution whose standard deviation $\sigma_\textrm{TAC}$ is user-defined and can be sampled at random in each simulation:
...
$\alpha^{\textrm{B}} = \frac{4\widetilde{h}}{(1-\widetilde{h})\phi_f^0}$,
$\beta^{\textrm{B}} = \frac{5\widetilde{h}-1}{(1-\widetilde{h})B_0}$
and
$\alpha^{\textrm{R}} = \frac{(5\widetilde{h})^{1.25}}{\phi_f^0}$,
$\beta^{\textrm{R}} = \frac{\log(5\widetilde{h})}{B_0}$,
where $B_0$ is unfished spawning biomass, derived from $\widetilde{R}_0$ (Equation \@ref(eq:dlm-om-bo)), and $\phi_f^0$ is unfished spawning biomass-per-recruit defined by setting $F^e=0$ in Equation \@ref(eq:dlm-om-phif).
...
<!--TODO Need to make sure TACFrac and TACSD are correctly described. See correspondence with Quang-->
...
"Obs.",     "CAA\\_ESS",  "$\\textrm{ESS}_\\textrm{LB}{CAA}$", "Effective sample size of multinomial catch-at-age observation error model.",
