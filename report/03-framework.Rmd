...
mse <- DLMtool::Sub(mse2, MPs = c(".Itarget_d0.8", ".Itarget_w0.8", ".Itarget_base"))
...
gfdlm::plot_convergence(mse, c("LT USR")) +
...
```{r methods-index, fig.cap="(ref:fig-methods-index)", out.width="4.5in"}
...
```{r methods-radar, fig.cap="(ref:fig-methods-radar)", out.width="4.5in"}
pm_df %>% 
  mutate(MP =  c("MP-1", "MP-2", "MP-3")) %>% 
  gfdlm::plot_radar(palette = "Dark2")
```

(ref:fig-methods-dots) dots TODO

```{r methods-dots, fig.cap="(ref:fig-methods-dots)", out.width="4.5in"}
map(pm_df_list_all, dplyr::filter, MP %in% c("CC1.2", "CC1.0", "CC0.8", "FMSYref75")) %>%
  map(~ {.x$MP <- c("MP-1", "MP-2", "MP-3", "FMSYref75");.x}) %>% 
  gfdlm::plot_dots(type = "single") + 
    scale_colour_manual(values = c("grey50", RColorBrewer::brewer.pal(3, "Dark2")))
```

(ref:fig-methods-tradeoff) trade-off TODO

```{r methods-tradeoff, fig.cap="(ref:fig-methods-tradeoff)", out.width="4.5in"}
...
purrr::map(seq_along(mse_all), ~ DLMtool::Sub(mse_all[[.x]], MPs = c(".Itarget_d0.8"))) %>%
  ...
  gfdlm::plot_scenario_projections()
```
