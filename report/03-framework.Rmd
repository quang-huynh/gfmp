...
source(here::here("analysis/rex/merge_MSE.R"))
...
mse2 <- readRDS(here::here("generated-data", "rex-mse-ind-ceq200.rds"))
...
```{r methods-tigure, fig.cap="(ref:fig-methods-tigure)", fig.asp=1.5, out.width="3.5in", fig.width=4}
mse2 %>%
  gfdlm::get_probs(PM) %>%
  mutate(MP = paste0("MP-", seq_len(length(MP)))) %>%
  gfdlm::plot_tigure(satisficed = c("LT LRP" = 0.9, "LTC" = 0.7))
```

(ref:fig-methods-dots) dots TODO

```{r methods-dots, fig.cap="(ref:fig-methods-dots)", out.width="6in"}
map(pm_df_list_all, dplyr::filter, MP %in% c("CC1.2", "CC1.0", "CC0.8", ".Itarget_x0.2", "FMSYref75")) %>%
  map(~ {.x$MP <- c("MP-1", "MP-2", "MP-3", "MP-4", "FMSYref75");.x}) %>%
  gfdlm::plot_dots(type = "single") +
    scale_colour_manual(values = c("grey50", RColorBrewer::brewer.pal(4, "Dark2")))
```

(ref:fig-methods-tradeoff) trade-off TODO

```{r methods-tradeoff, fig.cap="(ref:fig-methods-tradeoff)", fig.width=6, out.width="4.5in"}
pm_df_list_all_rob$`no-cpue-light` %>%
  ...
  gfdlm::plot_tradeoff("LT LRP", "STC") +
    scale_colour_brewer(palette = "Dark2") +
    coord_cartesian(xlim = c(0.6, 1), ylim = c(0.2, 1), expand = FALSE)+
  guides(shape = "none")
```
...
(ref:fig-methods-kobe) B/B~MSY~ and F/F~MSY~ values from the final year of the projections across all replicates.
The dots represent individual replicates.
...
For example, the 0.50 (outermost) contour lines encompass approximately 50% of the replicates.

```{r methods-kobe, fig.cap="(ref:fig-methods-kobe)", fig.asp=1.2, fig.width=4, out.width="3.5in"}
x <- DLMtool::Sub(mse, MPs = c(".Itarget_d0.8", ".Itarget_base"))
...
(ref:fig-methods-proj-sens) TODO: update! This visualization illustrates the sensitivity of some quantity of interest (y-axis; here B/B~MSY~) to high and low values (colours) of various OM parameters (columns) over time.
A variety of MPs are illustrated across rows.
The solid lines correspond to median values and shaded ribbons correspond to some quantile (here 0.15 and 0.85) to indicate variability across replicates.
For each OM parameter value and MP, the replicates from the top and lower third quantiles have been retained and the middle third has been discarded.
...
```{r methods-proj-sens, fig.cap="(ref:fig-methods-proj-sens)", out.width="\\textwidth", fig.asp=0.3}
