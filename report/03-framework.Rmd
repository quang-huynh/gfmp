...
An alternative would be... ICES each and every... TODO
...
We present performance metrics from the reference and robustness sets separately.
For most visualisations, the reference set performance metrics are averaged across all reference set scenarios. An exception is the table of performance metrics (e.g., Figure \@ref(fig:rex-tigure-avg)), which is presented in two ways: (i) minimum value of the performance metric across all reference set scenarios; and (ii) average value of the performance metric across all reference set scenarios.
The first case is a "worst-case scenario" approach, whereas the second case integrates across the whole reference set.

Performance metrics from the robustness set scenarios are presented for individual robustness scenarios.
This facilitates visualization of performance of MPs that perform well in the reference set under more diverse assumptions [@rademeyer2007].
...
This visualization demonstrates bivariate trade-offs between two performance metrics for a given scenario.
You can also be used for average performance across scenarios.
Bottom and left axis correspond to two performance metrics an individual dots represent MPs.

```{r methods-tradeoff, fig.cap="(ref:fig-methods-tradeoff)", out.width="3.5in"}
