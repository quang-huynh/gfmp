...
where $B_{y}$ is the sum over ages of the weight-at-age $W_a$, and numbers at age $N_{y,a}$:

\begin{equation}
B_{y} = \sum_{a = 1}^{n_{a}}{W_{a}N_{y,a}}
(\#eq:dlm-om-by)
\end{equation}
...
This approach means that the user can specify an unbiased time series (e.g., low
$\sigma_{\text{bI}}$ and therefore sampled values of
${\widetilde{b}}_{I}$ close to 1), or a biased time series (e.g., high
$\sigma_{\text{bI}}$ and therefore sampled values of
${\widetilde{b}}_{I}$ substantially lower or higher than 1). 
The time series can also be observed with a low degree of error (e.g., low sampled values
...
Observations of annual catch can be calculated in the same way (using parameters $\sigma_{\text{bC}}\ {,\widetilde{b}}_{C}$, ${\widetilde{\sigma}}_{C}$,
*LB~C~* and *UB~C~*)).
In the current framework for Pacific groundfish species, where groundfish fisheries are subject to 100% at-sea and dockside observer coverage, we make the assumption that catches are observed without error.
