...
where $F_{y,a}$ is fishing mortality-at-age, defined in Equation \@ref(eq:dlm-om-Fay)), and $v_{a}$ is vulnerability-at-age, defined in Equation \@ref(eq:dlm-om-va).
...
where the vulnerability-at-age $v_{a}$ is calculated from a length-based double-normal selectivity curve:
...
This will affect other model variables that depend on length, including weight-at-age (Equation \@ref(eq:dlm-om-wta)) maturity-at-length and age (Equations \@ref(eq:dlm-om-matl) and \@ref(eq:dlm-om-mata)), and vulnerability-age (Equation \@ref(eq:dlm-om-va).
...
How to treat reference points when parameters, particularly productivity parameters such as $M$ or $\kappa$, are time-varying is an active area of research [e.g., @haltuch2009; @haltuch2019; @punt2014]. We suggest that the MP Framework is a suitable environment for further simulation-testing of alternative approaches for time-varying reference points, and suggest this is an important research area for future applications of the MP Framework.
