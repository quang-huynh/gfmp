...
In this case, the index in years $t_1$ to $t_c$ is specified as data and the index $I$ (without observation error) in year $y$ (for $y > t_c$) and for survey $s$ is calculated as:

$$
I_{y,s} = \sum_{a=1}^A v_{a,s} N_{y,a} w_{y,a},
...
$$
I_{y,s} = \sum_{a=1}^A v_{a,s} N_{y,a},
(\#eq:sra-om-iysb-addind)
$$

for an abundance-based index.
The selectivity-at-age ($v_{a,s}$) is modelled using the same logistic form as for the commercial fleet (Equations \@ref(eq:dlm-om-va)--\@ref(eq:dlm-om-sigldes)) but with its own shape as specified by the user or estimated via the SRA.
