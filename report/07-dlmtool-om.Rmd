...
The closed-loop simulation, with application of the MPs and calculation of performance metricss begins in the first year of the projection period (year $t_{c+1}$).
...
where $N_{y,a}$ is annual numbers-at-age, given in Equation \@ref(eq:eq:dlm-om-nya)
...
where $A$ is the maximum age class and the annual total mortality rate $Z_{y,a}$ is the sum of the instantaneous rates of natural mortality ($M$), assumed constant across years and ages unless otherwise specified, and fishing mortality-at-age ($F_{y,a}$, defined in Equation \@ref(eq:dlm-om-Fay)):

$$
Z_{y,a} = M + F_{y,a}v_{a}
(\#eq:dlm-om-zya)
$$

where $v_{a}$ is vulnerability-at-age, defined in Equation \@ref(eq:dlm-om-va).

Note that Equation \@ref(eq:dlm-om-nya) treats the maximum age class $A$ as a plus group. This is not the default in DLMtool, which usually makes the assumption that $A$ is large enough to include all age classes (i.e., $N_{y-1,a-1}e^{-Z_{y-1,a-1}}$ is used for all age classes including $a = A$). However, when the model is conditioned using the SRA (Appendix \@ref(app:sra)), this model does default to treating $A$ as a plus group (Equation \@ref(eq:sra-om-nya)). Therefore, to ensure consistency between the historical and projection periods, the SRA function `SRA_scope()` sets DLMtool to treat $A$ as a plus group (setting `OM@cpars$plusgroup` = TRUE) for the closed-loop simulations. Since we recommend always conditioning OMs for BC groundfish using the SRA, we include the plus group in Equation \@ref(eq:dlm-om-nya).
...
To distribute catches over ages it is first necessary to calculate the distribution of vulnerable biomass across ages ($B^V_{y,a}$):

$$
B^V_{y,a} = N_{y,a} W_{a} v_{a} R_{a}
(\#eq:dlm-om-vulcom)
$$

where the vulnerability-at-age $v_{a}$ is calculated from a double-normal selectivity curve and length-at-age:

$$
v_{a} = \left\{ \begin{matrix}
2^{- \ \frac{{(L_{a} - {{L}}_{\textrm{smax}})}^{2}}{\sigma_{\textrm{sasc}}^{2}}} & L_{a} \leq {{L}}_{\textrm{smax}} \\
2^{- \ \frac{{(L_{a} - {{L}}_{\textrm{smax}})}^{2}}{\sigma_{\textrm{sdesc}}^{2}}} & L_{a} > {{L}}_{\textrm{smax}} \\
\end{matrix} \right\} ,
(\#eq:dlm-om-va)
$$
...
DLMtool allows users to allow a number of key parameters to vary through time, according to user-specified slope parameters . 
...
This will affect other model variables that depend on growth, including maturity-at-length and age, and vulnerability-at-length and age.
...
Time-varying growth, natural mortality and/or selectivity will result in time-varying MSY-based reference points, because DLMtool calculates $\textrm{MSY}$} and related metrics (e.g., $B_\textrm{MSY}$}) at each time-step in the model. It is possible to base performance metrics on an average $B_\textrm{MSY}$ from the historical period. 

We recommend further simulation-testing of both options in future applications of the MP Framework if time-varying parameters are thought to be important.

## DLMTOOL SLOTS

*TODO. EK is making a table for the slots. Not sure if we need to put this under a heading or whether it is just a table in this appendix*
