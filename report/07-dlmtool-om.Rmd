...
However, when the model is conditioned using the SRA (Appendix \@ref(app:sra)), the SRA defaults to treating $A$ as a plus group (Equation \@ref(eq:sra-om-nya)) and, to ensure consistency between the historical and projection periods, the SRA function forces DLMtool to also treat $A$ as a plus group for the closed-loop simulations in the projection period (by setting `OM@cpars$plusgroup` = TRUE). Since we recommend always conditioning OMs for BC groundfish using the SRA, we include the plus group in Equation \@ref(eq:dlm-om-nya).
...
where the selectivity-at-age $v_{a}$ (or vulnerability-at-age) is calculated according to whether users define selectivity to be asymptotic (logistic) or dome-shaped, using the following length-based function:
...
However, if the OM is calibrated using the SRA model (Appendix \@ref(app:sra)), the SRA can accommodate multiple fleets, and selectivity is fleet-specific, defined by user-defined settings for $L_5$, $L_{FS}$ and $V_{L_\infty}$ for each fleet (or estimated if age- or length-composition data are provided).
In this case, selectivity in the projection period is replaced in Equation \@ref(eq:dlm-om-zya) with the SRA-calibrated estimate of fishing mortality-at-age in the final year ($t_c$) of the historical period ($\Sigma_f v_{a,f} F_{t_c,f}$), normalized by dividing by apical $F$ in that year. 
This provides fractions of $F$-at-age consistent with the relative selectivity-at-age across fleets. 
...
The DLMtool OM only accommodates one index (most data-limited MPs rely on only one index). In this case, the SRA passes all of the indices back to DLMtool, preserving the estimated or fixed selectivity-at-age for each index. Users must select which index to use for the projection period.
...
Annual observed indices of abundance are calculated by adding observation error and bias to annual relative total biomass ($B^T_{y}$) through a term $\omega_{I,y}$ that includes bias and imprecision in the index observations:

$$
I_{y}^{\textrm{obs}} = \omega_{I,y}\frac{{B^T_{y}}^{\beta}}{\frac{1}{y}\sum_{i = 1}^{y}{B^T_{i}}^{\beta}}
(\#eq:dlm-om-iy)
$$
...
which is the average unfished spawning biomass over the first $\theta_{a50}$ years of the historical period, where age-at-50%-maturity ($\theta_{a50}$), rounded up to the nearest integer, is used as an approximation of generation time and $B_{unfished}^y$ is calculated $B_0$ (Equation \@ref(eq:dlm-om-bo)). Other alternative interpretations of $B_0$ could also be programmed (e.g., average $B_0$ across the whole historical period, or at the end of the historical period), depending on the desired benchmark or target.
