...
More complex structural assumptions, including time-varying parameters, could be developed as scenarios when justified.
...
The historical period is calibrated to historical observations.
...
2. Pass the OM parameters to the `SRA_scope` function in MSEtool, which fits an age-structured stock reduction analysis (SRA) [@kimura1982; @walters2006] to historical observed catches and indices of abundance (and to age- and length-composition data if available). 
...
Numbers-at-age are initialized in the first historical year using the SRA model (Appendix \@ref(app:sra)).

<!--
#RF COMMENTED OUT Feb 12 2020. Initial numbers and recruits for the historical period are estimated by the SRA and fully reported in the SRA appendix. 

Numbers-at-age are initialized in the first historical yearas a function of  \({{R}}_{0}\) and a vector of lognormal recruitment deviations $\varepsilon_{R,y}$:
...
\end{equation}
-->
During the projection period, the number of fish $N$ in each year $y$ > $t_c$  and age $a$ > 1 is calculated from the previous year and age class,
