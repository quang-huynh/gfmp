...
In this case, the true underlying index $I$ in year $y$ and for survey $s$ is calculated as:
...
The selectivity-at-age ($v_{a,s}$) is modelled using the same logistic form as for the commercial fleet (Equation \@ref(eq:dlm-om-va)) but with its own shape as specified by the user or estimated via the SRA.
...
$$
I_{y,s}^{\textrm{obs}} = \omega_{I,y,s}\frac{{I_{y,s}}^{\beta}}{\frac{1}{t_c}\sum_{i = 1}^{t_c}{I_{y,s}}^{\beta}},
(\#eq:dlm-om-iy-addind)
$$
