...
The DLMtool population dynamics operating model (OM) is an age-structured model decribed below.
...
Operating model development in the MP Framework using DLMtool follows three steps:
...
where $\theta_{\mathrm{AC}}$ represents a first-order autocorrelation term, and setting $\theta_\textrm{AC}$ to zero implies no autocorrelation.
...
The standard deviation parameters of the ascending limb are given by the length at 5% vulnerability $L_{5}$):
...
$$
\varepsilon_{I,y}\sim N\left( 0,{{\sigma}}_{I} \right)
(\#eq:dlm-om-epsiloni)
$$
...
$$
{\sigma}_{I}\sim U\left( \textrm{LB}_{I},\textrm{UB}_{I} \right).
(\#eq:dlm-om-sigi)
$$
...
annual effective sample size($C_{y,a}^{\textrm{obs}}$ or $C_{y,l}^{\textrm{obs}}$).
For example, $\textrm{ESS}_{CAA}$ independent catch
...
total sample size $n \cdot \textrm{CAA}$ and rounded to the nearest integer:
...
where ${{b}}_{\text{TAC}}$ is an improper fraction (e.g. ${{b}}_{\text{TAC}}$ = 0.7 is equivalent to 30% catch underages) and the lognormal error term $\varepsilon_{\textrm{TAC},y}$, is drawn from a standard normal distribution whose standard deviation ${\sigma}_{\textrm{TAC}}$ is sampled at random in each simulation:
...
${{\sigma}}_{\text{TAC}}$ from a uniform distribution.

$$
\sigma_{\textrm{TAC}}\sim U\left( \textrm{LB}_{\textrm{TAC}},\textrm{UB}_{\textrm{TAC}} \right)
(\#eq:dlm-om-sigmatac)
$$
...
$$
{{b}}_{\textrm{TAC}} = \exp\left( \varepsilon_{\textrm{bTAC}} - \frac{\sigma_{\text{bTAC}}}{2} \right)
(\#eq:dlm-om-btac)
$$

$$
\varepsilon_{\textrm{bTAC}}\sim N\left( 0,\sigma_{\textrm{bTAC}} \right)
