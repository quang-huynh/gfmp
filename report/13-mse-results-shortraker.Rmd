...
# SHORTRAKER ROCKFISH: MANAGEMENT PROCEDURE SPECIFICATIONS {#app:desc-mp-rex}
...
short_om@nsim <- 50
short_historical <- runMSE(short_om, Hist = TRUE, parallel = FALSE, ntrials=1000)
...
all_years <- data.frame(Year = (2018 - short_om@nyears):2018)
...
file_name <- here::here("generated-data", "short-mse.rds")
if (!file.exists(file_name)) {
  DLMtool::setup(cpus = floor(parallel::detectCores()/1))
  short_om@nsim <- 50
  short_mse <- runMSE(OM = short_om, MPs = mp$mp, parallel = TRUE, ntrials  =1000)
...
if (!file.exists(file_name)) {
  ...
  short_mse_short <- runMSE(OM = omsr, MPs = mpsr_short, parallel = TRUE, ntrials=1000)
