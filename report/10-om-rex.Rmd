...
*The maximum age of individuals that is simulated (there is no plus group). Positive integer.*
...
*A reference to a website or article from which parameters were taken to define the stock object.*
...
Based on the calculations described in @pennington2002 (TODO: explain elsewhere), the effective sample size for catch at length appears to be around 400.

```{r desc-obs-cal_ess-rex, results = knitr_results, echo = knitr_echo}
obs_rex@CAL_ESS <- c(300, 400)
```
<!-- slot-chunk-end -->
...
Set this to `c(1, 1)` unless we are specifically interested in hyperstability/hyper depletion.
