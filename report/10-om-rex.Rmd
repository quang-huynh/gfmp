...
@maunder2012 cite @myers1999 who found a median value of steepness of 0.80 for flatfish and note that, because of the bias in the value estimated for North Atlantic Cod in that paper, the value might be closer to 0.94. @grandin2017arrowtooth estimated steepness for Arrowtooth Flounder, but found their posterior was highly influenced by their prior (they estimated a median of 0.88 with a 95% confidence interval of 0.69--0.98, which nearly matches the prior). Here we will use the same prior used in @grandin2017arrowtooth but we also include an alternative scenario with higher steepness.
...
@thorson2014 found the mean autocorrelation for Pleuronectiformes to be about 0.44 with a standard deviation of about 0.27. We will use these values.
...
Although we do not have any ages in our database for WCVI, there exist a limited number of ages from the Hecate Strait Multispecies Assemblage Survey and a von Bertalanffy model fits reasonably well. We will extract samples from the model posterior and save them as custom parameters.

```{r rex-length1, echo=FALSE, fig.cap="von Bertalanffy model fit to lengths and ages from the Hecate Strait Multispecies Assemblage Survey. Dots represent individual fish and lines represent samples from the posterior."}
...
```{r desc-vb-parameters, fig.cap="von Bertalanffy parameter posteriors."}
...
Although we visualize the relationship for male and female fish separately, we fit a Bayesian model to sample parameter values from a model that estimates maturity for an average male/female fish from our samples.
...
```{r desc-lw-fits, fig.cap="Posterior samples of length at maturity parameters."}
...
```{r desc-stock-a-rex, results = knitr_results, echo = knitr_echo, fig.cap="Length-weight relationship for Rex Sole sampled from the WCVI Synoptic Survey with both sexes combined."}
...
abline(v = quantile(L50, probs = 0.025), col = "red")
abline(v = quantile(L50, probs = 0.975), col = "red")
```

We will assume that Rex Sole is fully vulnerable to fishing by approximately size at maturity:
