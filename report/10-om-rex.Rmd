...
We can look at a comparison of the commercial (from the entire coast) and survey samples (from WCVI) with the commercial samples shown is a histogram in the survey samples shown as line density. We will indicated the lower and upper bounds of the `L50` (length at 50% maturity) slot with red lines.
...
We will assume this is 1/4 of `Linf` for now. TODO: Ask industry.
...
We will assume this is 1/2 of `Linf` for now. TODO: Ask industry.
...
We do not have any length data on discards but assume that they will be retained if they are large (at `Linf`).
...
We looked at the discard rate since observer coverage on the trawl fleet (1996). We set the bounds as the mean +/ 1 standard deviation as observed in the data:
...
catch2 <- drex$catch %>%
  ...
  dplyr::rename(Landed = value) %>%
  dplyr::select(-gear)

discard_rate2 <- dplyr::left_join(discards2, catch2) %>%
  dplyr::mutate(frac_disc = Discards / (Landed + Discards))
