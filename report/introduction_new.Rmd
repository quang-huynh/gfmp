...
of biological reference points (BRPs), which define biomass targets as well as low biomass thresholds to be avoided with high probability. The approach requires that fishing mortality be adjusted in relation to two levels of stock status---a Limit Reference Point (LRP) and Upper Stock Reference (USR)---that delineate when fishing mortality is reduced or ceased (Figure \@ref(fig:fig-pa)). The LRP and USR delineate three stock status zones ("Healthy", "Cautious" and "Critical"). A Target Reference Point may also be defined. Of particular importance is the LRP, which is defined as the spawning biomass below which serious harm may occur to the stock. Other key elements of the PA Framework include a harvest control rule (HCR), which determines allowable removal rates in each of the three stock status zones, and the need to take into account risk and uncertainty when developing BRPs and determinining stock status in relation to BRPs [@dfo2006, @dfo2009].
...
In 2016, a CSAS workshop was held in the Pacific region to review international tiered approaches, as a preliminary evaluation of the suitability of a tiered approach for the BC integrated groundfish fishery [@dfo2016tiered]. Instead of recommending a discrete tiered approach, as described above, the workshop recommended considering data-richness on a continuous scale and simulation-testing multiple management procedures (MPs) on a stock-by-stock basis to identify data-limited methods that would best meet policy and fishery objectives, explicitly accounting for risk. The simulation-testing was recommended to be done using a closed-loop simulation, or management strategy evaluation (MSE) approach.

## MANAGEMENT-ORIENTED APPROACHES

In the scientific literature and in practice around the world, there has been a movement towards management-oriented approaches to stock assessment and fisheries management [e.g., @butterworth1999; @rademeyer2007; @berkson2015; @geromont2015; @punt2016; @carruthers2016]. Whereas traditional stock assessment traditionally focuses on estimation of biological parameters, a management-oriented, or management-strategy-evaluation (MSE), approach focuses on identifying MPs that perform best with respect to agreed-upon management objectives when implemented in a closed-loop simulation environment (Figure TODO). Importantly, closed-loop simulation incorporates feedback between the MP and the underlying system, represented by one or more operating models (OM). The MP may be a simple empirical rule such as an adjustment to catch based on a change in index of abundance, or may represent a more complex set of choices including data collection, stock assessment method, and HCR. 

*TODO: add a simple Closed-Loop Simulation figure here*

The scientific literature reports a great number of data-limited MPs, which have been shown to have variable performance across conditions such as species life history and levels of biomass depletion [e.g., @carruthers2014; @carruthers2016]. Closed-loop simulation testing of data-limited MPs allows for evaluation of relative performance of MPs across a range of uncertainties in, for example, underlying fish biology, state of depletion, observation error, estimation error, and implementation error [e.g., @kell2006; @carruthers2016]. Importantly, a management-oriented approach enables ranking of MPs with respect to meeting objectives, and enables evaluation of trade-offs among competing objectives (e.g., conservation and economic objectives).

A toolkit for applying closed-loop simulation for data-limited MPs has recently been developed by researchers at the University of British Columbia (UBC), in partnership with the Natural Resources Defense Council, DFO and other partners [@carruthers2018]. The Data-Limited Toolkit, "[DLMtool](https://www.datalimitedtoolkit.org/)" enables rapid assessment of a large number of data-limited MPs and ranking of MPs according to to identified conservation and economic objectives (e.g., probability biomass is below the LRP; long-term average catch). DLMtool is [open-source software](https://CRAN.R-project.org/package=DLMtool), enabling experienced users to customize MPs and performance metrics as needed.

## OBJECTIVES OF THIS DOCUMENT

This project aims to develop a management-procedure framework for selecting methods for provision of catch advice for Pacific groundfish stocks that do not have sufficient data for full statistical stock assessments. In doing so, it aims to improve the capacity of DFO to meet the Fish Stocks provisions of the *Fisheries Act* for data-limited species. The management procedure framework will apply closed-loop simulation to formalize the process of testing and selecting MPs for data-limited groundfish stocks, which will support the provision of scientific advice to fisheries managers. The project will focus on developing appropriate operating models, testing suites of management procedures, and determining management procedures that best meet the objectives of fisheries management and stakeholders for some example stocks with different life-histories. This work builds on recent work by @anderson2019synopsis, who developed a comprehensive, reproducible data synopsis for all Pacific groundfish species encountered by DFO surveys and commercial fisheries in BC. Data and empirical analyses from the synopsis are used to develop operating models within the DLMtool toolkit [@carruthers2018], which is then used for simulation testing of alternative data-limited MPs against performance metrics.

## EXPECTED BENEFITS

There are a number of expected benefits of this project. This management procedure approach is focused on identifying MPs for data-limited stocks that achieve long-term performance objectives. The closed-loop simulation approach will allow analysts to use the available data for specific stocks rather than prescribing one MP per data tier, as in previously published "tiered" approaches. Compared to the current status quo, this framework will allow DFO to provide scientific catch advice for more stocks; will develop a standardized and transparent approach across stocks; will reduce the turnaround time between requests for science advice and provision of advice; and will help build an understanding of the most important data needs and research priorities for reducing uncertainty in stock advice. Importantly, the framework will demonstrate methods to improve capacity for DFO Science to provide catch advice for data-limited stocks that is consistent with Canada's PA Framework, and therefore bring data-limited stocks into compliance with the Fish Stocks provisions of the *Fisheries Act*.

*TODO: RF to finish editing from here:*
