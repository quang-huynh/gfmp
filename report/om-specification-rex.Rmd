...
stock_rex@M <- c(0.17, 0.17) # TODO add uncertainty?
```
<!-- slot-chunk-end -->
...
@thorson2014 finds the mean standard deviation of log recruitment deviations for Pleuronectiformes to be about 0.64 with a standard deviation of about 0.26. We will set our uniform distribution to be approximately plus or minus one standard deviation.
...
@turnock2005 (Table 4.12) found the $L_\mathrm{inf}$ for males was 39.5 cm and $L_\mathrm{inf}$ for females was 44.9 cm in the Gulf of Alaska. This looks roughly in line with the length samples in our database. We will extend these values slightly in our operating model:

```{r desc-stock-linf-rex, results = knitr_results, echo = knitr_echo}
stock_rex@Linf <- c(38, 46)
```
<!-- slot-chunk-end -->
...
@turnock2005 (Table 4.12) in the Gulf of Alaska estimated $k$ to be 0.38 and 0.31 for males and females, respectively. We will extend those values slightly:

```{r desc-stock-k-rex, results = knitr_results, echo = knitr_echo}
stock_rex@K <- c(0.3, 0.4)
```
<!-- slot-chunk-end -->
...
@turnock2005 (Table 4.12) in the Gulf of Alaska estimated $t_0$ to be 0.79 and 0.69 for males and females, respectively. We will extend those values slightly:

```{r desc-stock-t0-rex, results = knitr_results, echo = knitr_echo}
stock_rex@t0 <- c(0.65, 0.85)
```
<!-- slot-chunk-end -->
...
We have sufficient maturity and length data to fit a logistic maturity model:
...
We have sufficient length and weight to fit model to our data for both sexes combined:
...
Since there is 100% observer coverage we can assume there are no unreported discards. Therefore the following slot does not get used.
...
We are assuming no discarding.

```{r desc-fleet-lfr-rex, results = knitr_results, echo = knitr_echo}
fleet_rex@LFR <- c(NA_real_, NA_real_)
```
<!-- slot-chunk-end -->
...
fleet_rex@Rmaxlen <- c(NA_real_, NA_real_)
```
<!-- slot-chunk-end -->
...
This lot will not be used in the main operating model since none of the MPs make use of age data.
...
```{r desc-obs-rex, results = knitr_results, echo = knitr_echo}
obs_rex@Iobs <- c(average_cv, average_cv)
```
<!-- slot-chunk-end -->
...
obs_rex@Btbiascv <- obs_rex@Ibiascv
```
<!-- slot-chunk-end -->
<!-- slot-chunk-begin -->
...
obs_rex@LenMbiascv <- c(0.05, 0.10)
```
<!-- slot-chunk-end -->
...
obs_rex@Linfbiascv <- c(0.05, 0.10)
```
<!-- slot-chunk-end -->
...
obs_rex@FMSYbiascv <- c(0.05, 0.20)
```
<!-- slot-chunk-end -->
...
obs_rex@FMSY_Mbiascv <- c(0.05, 0.20)
```
<!-- slot-chunk-end -->
...
obs_rex@BMSY_B0biascv <- c(0.05, 0.20)
```
<!-- slot-chunk-end -->
...
obs_rex@Irefbiascv <- c(0.05, 0.20)
```
<!-- slot-chunk-end -->
...
obs_rex@Brefbiascv <- c(0.05, 0.20)
```
<!-- slot-chunk-end -->
...
obs_rex@Crefbiascv <- c(0.05, 0.20)
```
<!-- slot-chunk-end -->
