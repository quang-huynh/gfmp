...
*TO DO: check discards in the database. This species has no TAC so maybe some are discarded* 
...
Look at the discard rate since observers on the trawl fleet (1996). The discard rate is quite high. Set the bounds as mean +/ 1 sd.

*QUESTION: If there was a quota introduced for Rex, would discarding stop?*

```{r desc-fleet-dr-rex, results = knitr_results, echo = knitr_echo}
discards <- readRDS(here::here("report/data/rex_catch_discards.rds"))

discard_rate <- discards %>% 
  dplyr::filter(year>1995) %>% 
  mutate(frac_disc=discarded_t/(landed_t+discarded_t))
  
mean_frac_d <- mean(discard_rate$frac_disc)
sd_frac_d <- sd(discard_rate$frac_disc)
mean_frac_d
sd_frac_d

frac_low <- mean_frac_d - sd_frac_d
frac_high <- mean_frac_d + sd_frac_d


fleet_rex@DR <- c(frac_low, frac_high)
```
<!-- slot-chunk-end -->
