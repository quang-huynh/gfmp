...
From @munk2001, who document the maximum ages obsrexerved in various surveys in Alaska, note the maximum age obsrexerve for Rex Sole in the Gulf of Alaska is 27 years. We don't have ages in our database to inform the maximum age for this region.
...
Since there is 100% obsrexerver coverage we can assume there is no unreported discards. Therefore the following slot does not get used.
...
Based on the obsrexerved commercial length samples let's assume that Rex Sole is fully vulnerable to fishing by the like that 50% maturity. FIXME
...
At least since 1996, obsrexervation error on catch should be very small. Setting this to 0.

TODO: does this affect historical build up because there was certainly much larger obsrexervation error on catch before 1996.

```{r desc-obs-rex, results = knitr_results, echo = knitr_echo}
obsrex@Cobsrex <- c(0, 0)
```
<!-- slot-chunk-end -->
<!-- slot-chunk-begin -->
...
We will keep the at 0 because obsrexervation error on catch should be very small with 100% on board and dockside monitoring.
...
```{r desc-obs-rex, results = knitr_results, echo = knitr_echo}
obsrex@Iobsrex <- c(average_cv, average_cv)
```
<!-- slot-chunk-end -->
<!-- slot-chunk-begin -->
...
```{r desc-obs-rex, results = knitr_results, echo = knitr_echo}
obsrex@Btobsrex <- obsrex@Iobsrex
```
<!-- slot-chunk-end -->
<!-- slot-chunk-begin -->
