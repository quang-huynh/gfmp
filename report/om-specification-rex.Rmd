...
# stock_rex@Linf <- c(-2, 2) * mvb_summary[,"Std. Error"][["linf"]] + mvb$pars[["linf"]]
...
# stock_rex@K <- c(-2, 2) * mvb_summary[,"Std. Error"][["k"]] + mvb$pars[["k"]]
...
# stock_rex@t0 <- c(-2, 2) *  mvb_summary[,"Std. Error"][["t0"]] + mvb$pars[["t0"]]
stock_rex@t0 <- c(0.69, 0.79)
```
<!-- slot-chunk-end -->
<!-- slot-chunk-begin -->
...
# stock_rex@LenCV <- sd2cv(exp(mvb$pars[["log_sigma"]]))
# round(stock_rex@LenCV, 2)
...
```{r warnings = FALSE}
fleet_rex <- DLMtool::Generic_Fleet # TODO: temporary
```
...
Based on the slot `isRel` below, this lot needs to be filled in proportion of size-of-maturity. The 95% CI of size at maturity from this analysis is `r stock_rex@L50[[1]]` to `r stock_rex@L50[[2]]`. The shortest length fish in the commercial samples is `r min_caught`, although commercial samples are only available from 2015 in WCVI.

```{r desc-fleet-l5-rex, results = knitr_results, echo = knitr_echo}
fleet_rex@L5 <- c(min_caught/stock_rex@L50[[2]], min_caught/stock_rex@L50[[1]])
```
<!-- slot-chunk-end -->
<!-- slot-chunk-begin -->
