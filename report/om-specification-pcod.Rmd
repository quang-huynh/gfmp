...
This operating model is for Pacific Cod in PFMC management areas 5ABCD (Queen Charlotte Sound, Hecate Strait and Dixon Entrance). Parameter values extracted from the 2018 stockpc assessment of Pacific Cod in Area 5ABCD [@forrest2019].
...
*Steepness of the stockpc recruit relationship. Uniform distribution lower and upper bounds. Values from 1/5 to 1*
...
*Type of stockpc-recruit relationship. Single value, switch (1) Beverton-Holt (2) Ricker. Integer*

Beverton-Holt as in stockpc assessment [@forrest2019].
...
*Current level of stockpc depletion SSB(current)/SSB(unfished). Uniform distribution lower and upper bounds. Fraction*
...
*A reference to a website or article from which parameters were taken to define the stockpc object. Single value. Character string.*
...
fleetpc <- read.csv(here::here("report/data/pcod-fleetpc-effort.csv"))
...
Based on the obspcerved commercial length samples this appears to be around or just slightly larger than the length at maturity.
...
*The name of the obspcervation model object. Single value. Character string.*
...
<!-- slot-chunk-begin -->
### Cobspc {#app:desc-obspc-pc}

*Log-normal catch obspcervation error expressed as a coefficient of variation. Uniform distribution lower and upper bounds. Non-negative real numbers*

At least since 1996, obspcervation error on catch should be very small.

TODO: does this affect historical build up because there was certainly much larger obspcervation are on catch before 1996.

```{r desc-obspcod-pc, results = knitr_results, echo = knitr_echo}
obspc@Cobspc <- c(0.05, 0.15)
```
<!-- slot-chunk-end -->
...
*Log-normal coefficient of variation controlling the sampling of bias in catch obspcervations for each simulation. Uniform distribution lower and upper bounds. Non-negative real numbers*

We will keep the possible sampling bias quite small because obspcervation error on catch should be very small with 100% on board and dockside monitoring.
...
*Number of catch-at-age obspcervation per time step. Uniform distribution lower and upper bounds. Positive real numbers*
...
*Effective sample size (independent age draws) of the multinomial catch-at-age obspcervation error model. Uniform distribution lower and upper bounds. Positive integers*
...
*Number of catch-at-length obspcervation per time step. Uniform distribution lower and upper bounds. Positive integers*
...
*Effective sample size (independent length draws) of the multinomial catch-at-length obspcervation error model. Uniform distribution lower and upper bounds. Positive integers*
...
*Log-normal coefficient of variation controlling error in obspcervations of relative abundance index. Uniform distribution lower and upper bounds. Positive real numbers*
...
<!-- slot-chunk-begin -->
### Btobspc {#app:desc-obspc-pc}

*Log-normal coefficient of variation controlling error in obspcervations of current stockpc biomass among years. Uniform distribution lower and upper bounds. Positive real numbers*

TODO: I'm not sure where this gets used are where we should draw this from. And how is this different from the index lots above? For now I will make it the same at `Iobspc`

```{r desc-obspcod-pc, results = knitr_results, echo = knitr_echo}
obspc@Btobspc <- obspc@Iobspc
```
<!-- slot-chunk-end -->
...
*Uniform-log bounds for sampling persistent bias in current stockpc biomass. Uniform-log distribution lower and upper bounds. Positive real numbers*
...
*Log-normal coefficient of variation for sampling persistent bias in obspcerved natural mortality rate. Uniform distribution lower and upper bounds. Positive real numbers*
...
*Log-normal coefficient of variation for sampling persistent bias in obspcerved growth parameter K. Uniform distribution lower and upper bounds. Positive real numbers*
...
*Log-normal coefficient of variation for sampling persistent bias in obspcerved t0. Uniform distribution lower and upper bounds. Positive real numbers*
...
*Log-normal coefficient of variation for sampling persistent bias in obspcerved maximum length. Uniform distribution lower and upper bounds. Positive real numbers*
...
*Log-normal coefficient of variation for sampling persistent bias in obspcerved length at first capture. Uniform distribution lower and upper bounds. Positive real numbers*
...
<!-- slot-chunk-begin -->
### Dobspc {#app:desc-obspc-pc}

*Log-normal coefficient of variation controlling error in obspcervations of stockpc depletion among years. Uniform distribution lower and upper bounds. Positive real numbers*
