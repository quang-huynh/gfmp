...
stock_pc@Mexp <- c(NA_real_, NA_real_)
```
<!-- slot-chunk-end -->
...
# TODO: Yikes! Need to check this delta method imp_pclementation.
...
For now, derive these as approximations of the effort timeseries from the CPUE standardization analyses. Use the time series from our "fleet_pc" that is most relevant to catching Pacific Cod.

```{r desc-fleet-effyears-pc, results = knitr_results, echo = knitr_echo}
fleet_pc <- read.csv(here::here("report/data/pcod-flt-effort.csv"))
all_years <- data.frame(year = (2018 - fleet_pc@nyears+1):2018)
fleet_pc <- dplyr::left_join(all_years, fleet_pc)
fleet_pc <- mutate(fleet_pc,
...
ggplot(fleet_pc, aes(year, ymin = effort_lower, ymax = effort_upper)) +
  geom_ribbon()

fleet_pc@EffYears <- fleet_pc$year
```
<!-- slot-chunk-end -->
...
fleet_pc@EffLower <- fleet_pc$effort_lower
```
<!-- slot-chunk-end -->
...
fleet_pc@EffUpper <- fleet_pc$effort_upper
```
<!-- slot-chunk-end -->
...
```{r warnings = FALSE}
obs_pc <- methods::new('Obs')
obs_pc <- DLMtool::Precise_Unbiased # TODO: temporary
```
...
Implementation should be very accurate for the groundfish fleet_pc so we will use the perfect imp_pclementation object and make it just slightly less perfect for now.
