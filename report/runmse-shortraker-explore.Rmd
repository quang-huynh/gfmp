...
Possibly something has changed in the OM? (Markdown commenting out not working, chunk is still being run)
...
probs <- get_probs(shortraker_mse, PMlist = c("P40", "P80", "PNOF", "AAVY", "LTY"))
plot_probs(probs)
```
...
```{r}
Tplot(shortraker_mse)
Tplot2(shortraker_mse)
```



```{r}
summary(shortraker_mse)
```

Illustrating the built-in plotting functions:

```{r}
wormplot(shortraker_mse)
```

```{r}
Pplot(shortraker_mse)

```

```{r}
Pplot2(shortraker_mse, traj="quant", quants=c(0.2, 0.8))
```

```{r}
Kplot(shortraker_mse)
```

```{r}
Cplot(shortraker_mse)
```
...
```{r}
 plot(shortraker_mse)
 oldpar <- par()
##DFO_hist(shortraker_mse)
par(mfrow = c(1, 1))
DFO_plot(shortraker_mse)
# DFO_plot2(shortraker_mse)
```

```{r, fig.asp=1, warning=FALSE}
DFO_proj(shortraker_mse)
#DFO_hist(shortraker_om)
# DFO_plot2(shortraker_mse)

# d <- DFO_plot2(shortraker_mse)
...
```{r, fig.asp=1}
yend <- max(shortraker_mse@proyears)
ffmsy <- shortraker_mse@F_FMSY[,,yend]
bbmsy <- shortraker_mse@B_BMSY[,,yend]
...
n <- data.frame(mp = seq_along(shortraker_mse@MPs), mp_name = shortraker_mse@MPs)
...
ffmsy <- shortraker_mse@F_FMSY[,,] %>% reshape2::melt() %>%
  dplyr::rename(iter = Var1, mp = Var2, ffmsy = value, year = Var3)
bbmsy <- shortraker_mse@B_BMSY[,,] %>% reshape2::melt() %>%
...
# pdf("analysis/shortraker-bbmsy-ffmsy-ts-ex.pdf", width = 6, height = 3.5)
# gridExtra::grid.arrange(p1, p2, ncol = 2)
# dev.off()
...
```{r, fig.asp=1.2}
voi_out <- DLMtool::VOI(shortraker_mse)
```
