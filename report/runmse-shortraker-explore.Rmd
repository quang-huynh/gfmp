...
omsr@nsim <- 200
#saveRDS(omsr,file="C:/GitHub/gfmp/generated-data/omsr.RDS")
...
if (!file.exists(file_name)) {
  DLMtool::setup(cpus = parallel::detectCores())
  short_mse <- runMSE(OM = omsr, MPs = mpsr, parallel = TRUE, ntrials=1000)
...
probs <- get_probs(short_mse_short, PMlist = c("P40", "P80","P100", "PNOF", "AAVY", "LTY"))
plot_probs(probs)
```
...
```{r}
library(ggspider)
pm_probs <- eval_pm(short_mse_short,
             pm_list = list("LTY",
                            "P10",
                            "P40",
                            "P80",
