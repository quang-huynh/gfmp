...
## CONSTANT-CATCH MANAGEMENT PROCEDURES
...
### CC_hist and CC_hist20: Average historical catch
...
where $x$ lies between 0 and 1. The TAC is constant for all future projections. There are five variants of this procedure, which are the same besides the value of $x$:
...
## INDEX-SLOPE MANAGEMENT PROCEDURES
...
where $C_{y-1}$ is the catch from the previous year and $\alpha$ is the ratio of the mean index in the most recent two years of the time series and the mean index in 3--5 years before current time. Due to the biennial nature of the synoptic role surveys for any one area, we propose an alternative version of this MP, "Iratio6", which calculates the ratio with an additional year in the numerator so that there are always the same number of years with data in the numerator as the denominator:
...
and a version, "Iratio8", which uses 4 years in the numerator and denominator:
...
where $\delta_\textrm{min}$ is the most negative drop allowed in the relative biomass index before the fishery is closed that year (by default assuming $\textrm{TAC}_\textrm{Floor} = 0$). We set $\delta_\textrm{min} = -0.50$ as in Cox et al. (In press), but this could be tuned for individual stocks. The maximum increase in TAC is capped at $\delta_\textrm{max} = 0.25$ by default.
...
where $\lambda$ controls the degree of smoothing and can range between 0 and 1. Cox et al. (In press) used $\lambda=0.5$. We define these MPs for DLMtool as "IDX" and ($\delta_{\textrm{min}} = 0.5$, $\delta_{\textrm{max}} = 0.25$, and $\textrm{TAC}_\textrm{Floor} = 0$) and "IDX_smooth" (same as IDX with $\lambda = 0.5$ to split the difference between the potentially proposed TAC and the one implemented the previous year).
...
## INDEX-TARGET MANAGEMENT PROCEDURES
...
f <- function(avg_catch_hist, Irecent, Iave, x, Imulti, ...) {
...
### SBT1: "Southern bluefin tuna" relative biomass target management procedure

SBT1 is an MP that makes incremental adjustments to TAC recommendations based on the trend in relative abundance index levels (Fig. \@ref(fig:mp-sbt1-demo)). It is based on an MP that was developed for Southern Bluefin Tuna (REF). The TAC is calculated as:
...
where $\lambda$ is the slope of the log relative abundance index over the last $n$ years (default $n = 5$), and $K_1$, $K_2$, and $\gamma$ are arguments to the MP. We note that our implementation uses a regression with the log of the relative abundance index, whereas the implementation in DLMtool at the time of writing uses a regression in natural space. We estimate the slope in log space so that the slope represents a percent change in relative abundance and is therefore independent of the scale of the abundance index. By default, these parameters are set to $K_1 = 1.5$, $K_2 = 3$, and $\gamma = 1$. We propose including an alternative version with $\gamma = 0.66$ (decreases TAC more rapidly if the index is trending down) (Fig. \@ref(fig:mp-sbt1-demo)) and implementing an upper cap of 1.2 on the fraction that the TAC can be increased from one year to the next.
...
Biomass $B_t$ in year $y$ is calculated as
...
where $C_t$ is the observed catch and $P_y$ is the surplus production given by
...
We fix $n = 2$ to represent a symmetrical Schaefer (REF) production curve or $n = 1$ to represent a Fox (REF) production curve.
