...
where $n$ is the number of historical years, and $C_i$ is the catch in historical year $i$ with $i$ starting of the first historical year. For our purposes in British Columbia, we suggest alternatives using average catch from the last 20 years, which encompasses a time period after the implementation of trawl ITQs in 1996. We denote this MP "CC_hist20":
...
This MP adjusts the TAC based on the value of the relative abundance index in the current year divided by the relative abundance index mean and standard error  [@jardim2015] (Fig. \@ref(fig:mp-ici-demo)). There are two versions: ICI, which uses an asymmetric confidence interval; and ICI2, which uses a symmetrical confidence interval. In both cases, the TAC is calculated as:
...
- Divide the standard deviation by the square root of the number of available relative abundance index time steps to derive the standard error ($\sigma_I$).
