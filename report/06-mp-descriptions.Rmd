...
where $C_{y-1}$ is catch from the previous year, $\beta_I$ is the slope of a linear regression of the ln abundance index over the last $n$ years (default of $n = 5$), and $\lambda$ is a parameter between 0 and 1 that controls how quickly TAC is adjusted based on the slope of the index.
...
We set $\delta_\textrm{min} = -0.50$ as in [@cox2020], but this could be tuned for individual stocks.
...
We define these MPs for DLMtool as "IDX" and ($\delta_{\textrm{min}} = 0.5$, $\delta_{\textrm{max}} = 0.25$) and "IDX_smooth" (same as IDX but with $\lambda = 0.5$ to split the difference between the potentially proposed TAC and the one previously recommended).

$\textrm{TAC}_\textrm{Floor} = 0$ would ideally be set on a stock-by-stock basis to a reasonable value required to maintain other groundfish fisheries.
...
```{r}
# f_itarget <- function(TACtarg, Irecent, Itarg, Iave, w) {
#   I0 <- 0.2 * Iave
#   if (Irecent >= I0) {
...
# pars %>%
#   mutate(TACtarg = paste("TACtarg = ", sprintf("%3.0f", TACtarg))) %>%
#   mutate(Itarg = paste("Itarg = ", Itarg)) %>%
