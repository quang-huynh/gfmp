...
where $\textrm{TAC}^*$ is $(1-xx)$ multiplied by the mean catch from the past 5 years for the first projection year. In subsequent years, $\textrm{TAC}^*$ is the TAC from the previous year. $\lambda$ is a gain parameter, and $\beta I$ is the slope of the log relative abundance index over the past $n$ years (we have set $n = 6$).
...
We have additionally modified this MP to add a maximum proportional increase in TAC from one year to the next of 1.2. We denote our modified versions of "Islope" as "Islope_mod" combined with the numbers as indicated above ("Islope_mod1", "Islope_mod2", etc.).
