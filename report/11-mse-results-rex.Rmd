...
We define the performance metrics as:
...
```{r rex-spider-satisficed-mps-reference, fig.cap="Performance metrics for satisficed MPs for the individual OMs in the reference set.", out.width="\\textwidth"}
...
The sensitivity of SSB/SSB~MSY~ (y-axis) to high and low values (colors) of various OM parameters (columns) over time.
This example uses the "Catch eq. 100%" OM.
...
The sensitivity of F/F~MSY~ (y-axis) to high and low values (colors) of various OM parameters (columns) over time.
This example uses the "Catch eq. 100%" OM.
