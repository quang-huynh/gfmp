...
```{r}
mp <- readr::read_csv(here::here("report", "mp.txt"))
mp_list <- split(mp, mp$type)
csasdown::csas_table(mp)
```
...
```{r, fig.cap="Radar plots for the Rex Sole WCVI closed-loop simulation (index slope MPs). Performance metrics are indicated at the end of each 'spoke'. Coloured lines represent the various management procedures.", out.width="4.5in", fig.width=10}

filter(rex_satisficed_df, type == "Index slope") %>% 
  pull(mp) %>% 
  DLMtool::Sub(rex_mse, .) %>% 
  gfdlm::spider(pm_list = PM, palette = "Set2", lwd = 1.0)
```

```{r, fig.cap="Radar plots for the Rex Sole WCVI closed-loop simulation (index target MPs). Performance metrics are indicated at the end of each 'spoke'. Coloured lines represent the various management procedures.", out.width="4.5in", fig.width=10}
filter(rex_satisficed_df, type == "Index target") %>% 
  pull(mp) %>% 
  DLMtool::Sub(rex_mse, .)
  gfdlm::spider(pm_list = PM, palette = "Set2", lwd = 1.0)
```

```{r, fig.cap="Radar plots for the Rex Sole WCVI closed-loop simulation (constant catch MPs). Performance metrics are indicated at the end of each 'spoke'. Coloured lines represent the various management procedures.", out.width="4.5in", fig.width=10}
filter(rex_satisficed_df, type == "Constant catch") %>% 
  pull(mp) %>% 
  DLMtool::Sub(rex_mse, .)
  gfdlm::spider(pm_list = PM, palette = "Set2", lwd = 1.0)
```
