...
Data are shown in Figures \@ref(fig:rex-data-1) and \@ref(fig:rex-data-2).
...
Prior to 1996, reported catches for WCVI Rex Sole were much lower than post-1995 and there were no reported discards (Figure TODO). Assumptions about pre-1996 catch levels affect the estimated depletion level in 1996 from the SRA model, which is used to calibrate the OMs to historical observations (Appendix \@ref(app:sra)). The SRA model parameter $\tilde{C}^{\textrm{eq}}_f$, describes the equilibrium catch in weight prior to the first year of the model (1996 for Rex Sole), which is used to calculate equilibrium fishing mortality prior to the first year of the model $F^{\textrm{eq}}_f$, which in turn is used to calculate total mortality $Z^{\textrm{eq}}_a$ needed to calculate recruits and numbers in the first year of the model (Equation C.3). We note that the use of $F^{\textrm{eq}}_f$ and $\tilde{C}^{\textrm{eq}}_f$ make the assumption that the population biomass and fishery were in equilibrium at that catch level. While this is likely an unrealistic assumption, it is a convenient means of initializing a population model in the presence of fishing. For this reason, we treated the value of $\tilde{C}^{\textrm{eq}}_f$ as a major axis of uncertainty.
...
"Catch eq. 200%" assumes that equilibrium catch prior to 1996 was twice that in 1996, possibly because it was not mandatory to report all landings and discards prior to 1996. 

"Catch eq. 200% + CPUE" also assumes that equilibrium catch prior to 1996 was twice that in 1996, plus the SRA model is fit to the commercial CPUE index, which is available from 1996, therefore providing a longer index of abundance than the synoptic bottom trawl survey (Figure \@ref(fig:rex-data-1)).

"Catch eq. 50%" assumes that equilibrium catch prior to 1996 was the same as that in 1996, approximating the reported 1995 catch. 

We considered two scenarios that differed 
