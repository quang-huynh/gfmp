...
```{r, fig.cap="Radar plots for the Rex Sole WCVI closed-loop simulation (constant catch and index-based MPs). Performance metrics are indicated at the end of each 'spoke'. Coloured lines represent the various management procedures.", out.width="4.5in", fig.width=10}
DLMtool::Sub(rex_mse, MPs = rex_satisficed[!grepl("^L", rex_satisficed)]) %>%
  gfdlm::spider(pm_list = PM, palette = "Set2", lwd = 1.0)
```

```{r, fig.cap="Radar plots for the Rex Sole WCVI closed-loop simulation (length-based MPs). Performance metrics are indicated at the end of each 'spoke'. Coloured lines represent the various management procedures.", out.width="4.5in", fig.width=10}
DLMtool::Sub(rex_mse, MPs = rex_satisficed[grepl("^L", rex_satisficed)]) %>%
  gfdlm::spider(pm_list = PM, palette = "Set2", lwd = 1.0)
```
