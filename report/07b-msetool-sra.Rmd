...
However, initial depletion is difficult to estimate with precision without other sources of information (e.g., a long index of abundance, or age-composition data).
...
If multiple fleets are used for conditioning, then vulnerability-at-age ($v_{a,f}$, Equation \@ref(eq:sra-om-vaf)) will be updated based on the relative fishing mortality among fleets.
...
Fleet-specific vulnerability-at-age ($v_{a,f}$) is length-based and modeled as a double-exponential function (using base 2).
...
where $R_y$ is the recruitment and $A$ is the maximum-age, treated as a plus-group. 

<!-- TODO: RF to add note about how plus group is handled in DLMtool if the SRA is used (Issue #94)-->

Annual recruitment is modelled as
...
where $\textrm{obs}$ and $\textrm{pred}$ indicate observed and predicted quantities, respectively, and $\lambda$ are likelihood weights. With a very small standard deviation for the catch likelihood relative to the variance in other likelihood components, the predicted catch should match the observed catch almost perfectly.
