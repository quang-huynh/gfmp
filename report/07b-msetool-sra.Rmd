...
Multiple surveys and fleets can be accommodated by the SRA. Including one of these data types in addition to the conditioning catch time series is generally needed to inform depletion estimates.
...
For example, an age-composition sample from a single recent year that shows a very truncated age structure can imply a heavily depleted stock. Age-composition data from one or a few years can also be informative about selectivity, which in turn informs estimates of stock size.
...
- Natural mortality using slots `OM@M, OM@M2` or `OM@cpars$M_ageArray`[$M$, $M_2$]

- Maturity using slots `OM@L50, OM@L50_95` or `OM@cpars$Mat_age` [$\theta_{l50}$, $\theta_{l50-95}$]

- Standard deviation of recruitment deviations using slot `OM@Perr` or `OM@cpars$Perr` [$\sigma_{R}$]
