...
where $C_{y-1}$ is the catch from the previous year and $I_\delta$ is the ratio of the mean index over the past 5 years to a reference index level. The maximum allowable annual change in TAC is determined by mc: e.g mc = 0.05 means that the maximum change in TAC from the previous catch is 5\%. IT5 and IT10 have mc set to 0.05 and 0.1 respectively.
...
If $L_\textrm{recent} \ge L_0$ the TAC is:
$$
...
There are five variants on this procedure:
...
There are 3 variants for this procedure:
...
There are four variants to this procedure:
