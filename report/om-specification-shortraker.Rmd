...
```{r desc-stock-m2, results = knitr_results, echo = FALSE}
stock@M2
```
...
```{r desc-stock-mexp, results = knitr_results, echo = FALSE}
...
```{r desc-stock-period, results = knitr_results, echo = FALSE}
stock@Period
```
...
```{r desc-stock-amplitude, results = knitr_results, echo = FALSE}
stock@Amplitude
```
...
stock@Size_area_1 <- c(0.499, 0.501)
```
...
stock@Frac_area_1 <- c(0.499, 0.501)
```
...
stock@Prob_staying <- c(0.499, 0.501)
```
...
We can assume that nearly 100% of the discarded shortraker rockfish die from barotrauma.
...
We don't have a lot of information to go on for this. There is likely some proportion of the population that is not vulnerable because of MPAs, the frozen footprint, and the extreme depths that the slow-moving nature of shortrocker rockfish live at.

```{r desc-fleet-vmaxlen, results = knitr_results, echo = knitr_echo}
fleet@Vmaxlen <- c(0.05, 0.95)
```
...
Since we can assume that no shortraker rockfish will survive if caught, we will set this to 0.
...
We will use the vector of selectivity breakpoints below to allow for historical changes. Therefore the we will leave this slot empty.
...
```{r desc-fleet-selyears, results = knitr_results, echo = FALSE}
fleet@SelYears
```
...
```{r desc-fleet-absselyears, results = knitr_results, echo = FALSE}
fleet@AbsSelYears
```
...
```{r desc-fleet-l5lower, results = knitr_results, echo = FALSE}
fleet@L5Lower
```
...
```{r desc-fleet-l5upper, results = knitr_results, echo = FALSE}
fleet@L5Upper
```
...
```{r desc-fleet-lfslower, results = knitr_results, echo = FALSE}
fleet@LFSLower
```
...
```{r desc-fleet-lfsupper, results = knitr_results, echo = FALSE}
fleet@LFSUpper
```
...
```{r desc-fleet-vmaxlower, results = knitr_results, echo = FALSE}
fleet@VmaxLower
```
...
```{r desc-fleet-vmaxupper, results = knitr_results, echo = FALSE}
fleet@VmaxUpper
```
...
```{r desc-fleet-mpa, results = knitr_results, echo = FALSE}
fleet@MPA
```
