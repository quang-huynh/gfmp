...
Run the historical part of the MSE and look at OM historical catch compared to observed catch ... should be the same for P Cod
...
ompc@nsim <- 100L #Why is this set again?
...
Check convergence. This is to answer the question of whether enough simulations have been run ... does the order of the MPs change across iterations? If so, increase the number of iterations.

```{r}
DLMtool::Converge(pcod_mse)
```

Seems to be okay but all the MPs give identical B/BMSY=100.

Now look to see if we have redundant MPs.

```{r}

nSim <- ompc@nsim
nYear <- ncol(pcod_mse@C[,1,]) #find global parameter for number of projection years
nMP <- length(mppc)
dimnames(pcod_mse@C) <- list(1:nSim, mppc, paste0("Year",1:nYear))

#convert to data frame
z1 <- apply(pcod_mse@C, 3L, c)
z2 <- expand.grid(dimnames(pcod_mse@C)[1:2])
mp_catch_pc <- data.frame(z2, z1) %>% 
  reshape2::melt()
colnames(mp_catch_pc) <- c("Sim","MP","Year", "Catch")

#Make boxplots of catch from similar MPs for a subset of years
ggcatch <- mp_catch_pc %>% 
  filter(MP %in% mppcg1) %>% 
  filter(Year %in% c("Year1","Year10","Year20","Year30","Year40","Year50")) %>%
  ggplot(aes(x=Year,y=Catch,fill=MP))+
  geom_boxplot()
print(ggcatch)
