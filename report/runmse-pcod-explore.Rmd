...
nMP <- length(mppc)
dimnames(pcod_mse_C) <- list(1:nSim, mppc, 1:nYear)
...
nMP <- length(mppc)
dimnames(pcod_mse_SSB) <- list(1:nSim, mppc, 1:nYear)
...
nSim <- ompc@nsim
nYear <- ncol(pcod_mse_SSB[,1,]) 
nMP <- length(mppc_short)
dimnames(pcod_mse_SSB_short) <- list(1:nSim, mppc_short, 1:nYear)
dimnames(pcod_mse_C_short) <- list(1:nSim, mppc_short, 1:nYear)
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
nSim <- ompc@nsim
RefYield_pc <- data.frame("Sim"=1:nSim, "MP"=rep("RefY",nSim),"Year"=rep(1,nSim),  "Catch"=pcod_mse_short@OM$RefY)

#HACK Remove an outlier ... limit refY to 1000 for plotting
MaxRefY <- max(RefYield_pc$Catch)
NumOutliers <- length(which(RefYield_pc$Catch>10000))
func <- function(x) {ifelse(x> 10000,10000,x)}
RefY_Pc_trunc <- sapply(RefYield_pc$Catch, FUN=func)

#Replace RefY with truncated set, just for plotting
RefYield_pc$Catch <- RefY_Pc_trunc

#Make a new object so we don't accidentally keep appending to mp_catch_pc_short
mp_catch_pc_short2 <- rbind(mp_catch_pc_short,RefYield_pc)

ggcatch <- mp_catch_pc_short2 %>% 
  filter(Year %in% c(1,10,20,30,40,50)) %>%
  ggplot(aes(x=as.factor(Year),y=Catch,fill=MP))+
  geom_boxplot()+
  geom_hline(yintercept=med_refY_pc, linetype="dashed", color = "black")+
  ggtitle("Shortlist of MPs: Catch showing ref yield")+ xlab("Year")+ ylab("Catch") +
  annotate(geom="text", x=4, y=9500, label=paste("Maximum RefY value =", round(MaxRefY,1))) +
  annotate(geom="text", x=4, y=9000, label=paste("Number of RefY sims > 10000 =", NumOutliers))  
print(ggcatch)
...
Look at projected fishing mortality. This is odd. The DD4010 MP produces much higher F than any other MP, yet the spawning biomass is the same across all MPs and catches are the same as the AvC MP, which doesn't have such high F.

```{r, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
pcod_mse_F_short <- pcod_mse_short@FM
dimnames(pcod_mse_F_short) <- list(1:nSim, mppc_short, 1:nYear)
...
dimnames(pcod_mse_VB_short) <- list(1:nSim, mppc_short, 1:nYear)
...
dimnames(pcod_mse_BBMSY_short) <- list(1:nSim, mppc_short, 1:nYear)

gg <- list()
lapply(1:nMP, function(x){
 tmp <- pcod_mse_BBMSY_short[,x,] %>% 
   reshape2::melt() 
...
dimnames(pcod_mse_BBMSY_short) <- list(c(1:nSim), mppc_short, 1:nYear)
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
pcod_mse_FFMSY_short <- pcod_mse_short@F_FMSY
dim(pcod_mse_FFMSY_short) #dimensions of array are 1:nSim, 1:nMP, 1:nYear
dimnames(pcod_mse_FFMSY_short) <- list(c(1:nSim), mppc_short, 1:nYear)

#convert to data frame and plot
z1 <- apply(pcod_mse_FFMSY_short, 3L, c)
z2 <- expand.grid(dimnames(pcod_mse_FFMSY_short)[1:2])
mp_ffmsy_pc_short <- data.frame(z2, z1) %>% 
  reshape2::melt()%>% 
  rename("Sim"=Var1,"MP"=Var2,"Year"=variable, "FFMSY"=value) %>% 
  mutate(Year=as.numeric(Year)) 


ggb <- mp_bbmsy_pc_short %>% 
  filter(Year %in% c(1,10,20,30,40,50)) %>%
...
=======
ggspider::spider_web(x, "mp", "pm", "prob")
>>>>>>> Stashed changes
