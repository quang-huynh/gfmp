...
Worldwide, there has been a movement towards management-oriented approaches to stock assessment and fisheries management [e.g., @butterworth1999; @plaganyi2007; @rademeyer2007; @berkson2015; @geromont2015; @punt2016; @carruthers2016].
...
Although the proposed framework is software agnostic, in the current implementation, data and empirical analyses from the synopsis are used to develop OMs using the DLMtool [@carruthers2018] and MSEtool packages [@huynh_msetool_2019], implemented in the statistical software R [@r2019].
...
Critically, the framework will develop methods to improve capacity for DFO Science to provide sustainable, data-limited catch advice, consistent with Canada's Precautionary Approach Framework as legislated under the Fish Stocks provisions, therefore enabling data-limited stocks to be brought into compliance with the Fish Stocks provisions of the *Fisheries Act*. The authors acknowledge that while the framework does not involve explicitly defining biological reference points—which  cannot be reliably estimated for data-limited stocks—the framework is designed to evaluate management procedures on the probability of maintaining stocks above implicit reference points over a range of uncertainties. 
