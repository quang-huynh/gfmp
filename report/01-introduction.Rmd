...
(ref:fig-mse-chart-basic) Illustration of the fisheries closed-loop simulation process. The management procedure may be based on a simple data rule (e.g., decrease the allowable catch x% if the survey index decreases y%) or it might be an estimation model combined with a harvest control rule.

```{r mse-chart-basic, fig.cap="(ref:fig-mse-chart-basic)", out.width="4in"}
knitr::include_graphics(here::here("images/mse-chart-simple.png"))
```
