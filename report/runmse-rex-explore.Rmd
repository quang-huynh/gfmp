...
x <- rex_historical@Data@CAL %>%
  ...
  transmute(
    sample_id = Var1, year = Var2 + min(all_years) - 1,
    proportion = value, type = "Simulated"
  ) %>%
  mutate(length_bin = rep(bins, each = max(sample_id) * length(all_years)))

filter(x, year %in% seq(1996, 2018, 2)) %>%
  filter(sample_id %in% sample.int(150, 9)) %>%
  ggplot(aes(length_bin, proportion)) +
  geom_col() +
...
satisficed <- filter(probs[[1]], P40_yrs36_50 >= 0.70) %>% pull(MP)
