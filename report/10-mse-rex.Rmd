...
We began by assessing average and minimum performance across all candidate MPs for the reference set of OMs (Figures \@ref(fig:rex-tigure-min) and \@ref(fig:rex-tigure-avg)) to determine which MPs would be carried forward as satisficed MPs [@miller2010].
To obtain a manageable number of MPs for further consideration, we set satisficing thresholds of `r names(satisficed_criteria)[1]` $>$ `r satisficed_criteria[[1]]` and `r names(satisficed_criteria)[2]` $>$ `r satisficed_criteria[[2]]` This left us with `r number_to_word(length(mp_sat))` MPs (`r gfutilities::commify(sort(mp_sat))`).
