...
We measure the performance metrics as the probability integrating over all years and replicates simultaneously.
...
<!-- Selectivity-at-length used by all OMs is shown in Figure \@ref(fig:desc-selectivity-function).
Selectivity-at-length is calculated from two parameters: $L^5$, the shortest length corresponding to 5% vulnerability; and $L^\textrm{FS}$, the shortest length that is fully vulnerable to fishing.
...
The reference and robustness scenarios rendered a range of these parameter values (Figures \@ref(fig:rex-sra-conditioned-parameters) and \@ref(fig:rex-F-om)).
...
csasdown::csas_table(mp, caption = "Candidate MPs. We used the `.` version of the MPs, where applicable, to refer to the version of the MPs that only observes the survey index every second year to mimic our biennial WCVI synoptic survey.", col_names = c("Management procedure", "MP type"))
```

We set evaluated a range of priors on the intrinsic rate of population increase $r$ in the surplus production models: $\textrm{Normal}(0.4, 0.1)$, $\textrm{Normal}(0.5, 0.1)$, and $\textrm{Normal}(0.6, 0.1)$.
With the Schaefer surplus production function, these correspond to $F_\textrm{MSY} = 0.2$, $F_\textrm{MSY} = 0.25$, and $F_\textrm{MSY} = 0.3$, which roughly mimic the values of natural mortality $M$ explored in the operating models. In applications of the Framework, additional tuning of the priors could be explored.
...
To obtain a manageable number of MPs for further consideration, we set satisficing thresholds of LT LRP $>$ 0.9 and STC $>$ 0.8.
