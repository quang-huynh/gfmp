...
(ref:fig-rex-sra-conditioned-parameters) Frequency polygons of parameters estimated as part of the SRA. These are effectively polygons outlining a histogram. Fishing mortality at age, the historical depletion trajectory, and historical recruitment deviations are also derived from the SRA model. Apical fishing mortality by year are shown in Figures \@ref(fig:rex-proj-ceq200)--\@ref(fig:rex-proj-no-cpue) and Figures \@ref(fig:rex-proj-inc-m)--\@ref(fig:rex-proj-inc-m), and historical depletion trajectories are shown in Figure \@ref(fig:rex-depletion-om).
...
We ran the closed-loop simulations across 250 stochastic replicates using DLMtool version 5.4.2 with the OMs and MPs described above.
...
To obtain a manageable number of MPs for further consideration, we set satisficing thresholds of LT LRP $>$ 0.9 and STC $>$ 0.8. This left us with `r length(mp_sat)` MPs (`r gfutilities::commify(sort(mp_sat))`).
...
We also checked that the satisficing rules had converged (the selection of satisficed MPs would not change with further replicates).
...
We can look at timeseries trajectories of B/B~MSY~, F/F~MSY~, and catch to further understand performance across the various MPs and reference set OMs (Figure \@ref(fig:rex-proj-ceq100)--\@ref(fig:rex-proj-low-h); other trajectories not shown yet).
...
We can also look at "Kobe" plots that demonstrate either the final B/B~MSY~ vs. F/F~MSY~ status of the various replicates or the trajectory of these status values through time (Figure \@ref(fig:rex-kobe) and \@ref(fig:rex-worm)).
