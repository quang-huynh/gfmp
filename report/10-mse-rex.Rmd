...
Rex Sole are a commonly caught flatfish species in BC groundfish bottom trawl surveys and although they are commonly caught in the commercial mixed-species bottom trawl fishery, there is no directed fishery or assigned quota for them. They are commonly caught with Pacific Cod, Dover Sole, English Sole and Arrowtooth Flounder. Rex Sole have never been formally assessed in BC; however, biomass of Rex Sole in Hecate Strait was estimated in 2012 using research survey data collected between 1984 and 2003 [@fargo2012].
...
The available data include TODO and reference figures
...
sc %>%
  ...
  csasdown::csas_table(caption = "Rex Sole OM scenarios. \"Ceq.\" refers to \"catch equilibrium\" in 1995 and is used as a way to parameterize the level of biomass depletion at the beginning of the recorded catch history.",
    col_names = c("Scenario name", "Category"))
```
...
The SRA model parameter $C^{\textrm{eq}}_f$, describes the equilibrium catch in weight prior to the first year of the model (1996 for Rex Sole), which is used to calculate equilibrium fishing mortality prior to the first year of the model $F^{\textrm{eq}}_f$, which in turn is used to calculate total mortality $Z^{\textrm{eq}}_a$ needed to calculate recruits and numbers in the first year of the model (Equation \@ref(eq:sra-om-ceqpred)).
We note that the use of $F^{\textrm{eq}}_f$ and $C^{\textrm{eq}}_f$ make the assumption that the population biomass and fishery were in equilibrium at that catch level prior to 1996.
...
For this reason, we treated the value of $C^{\textrm{eq}}_f$ as a major axis of uncertainty.

We considered three reference-set scenarios that varied in the assumed value of $C^{\textrm{eq}}_f$ (Table \@ref(tab:rex-scen)).

We attempted to fit $C^{\textrm{eq}}_f$ values ranging from 0 to 2.5 times the catch in 1996, which imply that equilibrium catch in 1995 was 0% to 250% of catch in 1996.
The SRA model estimated implausibly high values of fishing mortality $F$ ($F > 3$) if $C^{\textrm{eq}}_f < 200\%$ 1996 catch suggesting that there may be considerable missing discarded catch prior to 1996 and the stock may not have been close to $B_0$ in 1995.
...
First, standardized commercial CPUE shows an increasing trend from 1996 to 2003 before the SYN WCVI survey began (Figure \@ref(fig:REF)).
While there are many caveats to the commercial CPUE timeseries, two factors lead us to play some faith in late 1990s trend: (1) commercial CPUE has somewhat mimicked the broad patterns seen in the SYN WCVI survey index since the survey began (Figure \@ref(fig:REF)) and (2) Rex Sole have not been a targeted species by the commercial trawl fleet.
This increase in commercial CPUE from 1996 to 2003 suggests that there may be a substantial amount of missing discarded catch prior to 100% at-sea-observer coverage in 1996.
Second, the SYN WCVI survey has shown approximately a doubling in its relative index value from the early 2000s to 2019, which will be consistent with the stock starting in a somewhat depleted state in 1996.
Third, other species that are commonly caught along with (often discarded) Rex Sole in catch records since 1996 were reported caught in substantial quantities before 1996 (TODO FINISH).
It therefore seems reasonable to assume that there is missing discarded catch prior to 1996.
...
* The scenario "No CPUE Ceq. 250%" also assumes that equilibrium catch prior to 1996 was 2.5 times that in 1996, plus commercial CPUE index is excluded from the SRA model (Figure \@ref(fig:rex-surv-index-plot)). Values less than Ceq. 250% rendered implausible SRA model fits without the commercial CPUE ($F > 3$).

#### Steepness

We considered one alternative scenarios that differed in terms of the steepness of the stock-recruit relationship (Table \@ref(tab:rex-scen)).
...
* The scenario "Higher steepness" use the steepness $h$ value of 0.95.

#### Natural Mortality

We considered one alternative scenario that differed in terms of the value of natural mortality (Table \@ref(tab:rex-scen)).
Unless otherwise stated, all other scenarios drew from a baseline uniform distribution *M* = 0.17--0.25 *y*^-1^, which starts at the value of 0.17 *y*^-1^ used for the Gulf of Alaska stock assessment of Rex Sole (@mcgilliard2017, see Section \@ref(app:desc-stock-m-rex)).

* The scenario "Higher M" considers that *M* could be latitude-dependent and may be higher for stocks in warmer BC waters (*M* = 0.3 *y*^-1^).
...
Unless otherwise stated, all other scenarios drew from a length selectivity curve that approximately matched the maturity ogive ($L_5 = 22$, $L^\textrm{FS} = 32$).

* The scenario "Lower selectivity" considers that the selectivity curve could be shifted to the left resulting in both the survey and the commercial fleet capturing younger fish ($L_5 = 17$, $L^\textrm{FS} = 28$).
...
* The "No CPUE Ceq. 50%" scenario assumes that catch at equilibrium in year 1995 was 50% of the catch in 1996. In order to fit such a model, we also had to omit the commercial CPUE and use the selectivity curve described in the "Lower selectivity" scenario.
