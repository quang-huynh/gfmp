...
We considered three reference-set scenarios and one robustness-set scenario that varied in the assumed value of $C^{\textrm{eq}}$ (Table \@ref(tab:rex-scen)).
...
We considered the above six scenarios to represent the most plausible uncertainty scenarios for Rex Sole and grouped them into the "reference set" of OMs (see Section \@ref(sec:best3)).
...
* The "M increasing" scenario assumes that *M* was fixed at 0.2 in the historical period and increases monotonically from 0.2 to 0.4 *y*^-1^ over the projection period. This could represent increasing natural mortality due to a process such as climate change or shifts in predator abundance. We include this scenario simply to illustrate including a time-varying parameter in the OM projection period that reflects an underlying environmental process, and to assess robustness of MPs to such a scenario. We do not necessarily expect this to become a default robustness OM in applications of the framework.
...
The implied spawning stock biomass depletion trajectories during the historical period from the eight OMs were similar with the exception of the "No CPUE Ceq. 50%" scenario (Figure \@ref(fig:rex-depletion-om)).
...
This scenario estimates the stock to be smaller (smaller $R_0$ values in Figure \@ref(fig:rex-sra-conditioned-parameters)) with high $F_y$ (apical $F$ by year) values around 2005 (Figure \@ref(fig:rex-F-om)).
