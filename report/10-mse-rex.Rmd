...
Rex Sole (*Glyptocephalus zachirus*) occur from central Baja California to the western Bering Sea, and are generally distributed throughout coastal BC [@mecklenburg2002]. They have been caught in BC at depths between 20 m and greater than 1000 m, with most captured between 75 m and 450 m. Rex Sole have been caught in BC waters as large as 58 cm and up to 1.44 kg. The only aged Rex Sole from BC are those caught in the Hecate Strait Multispecies Assemblage Bottom Trawl survey in 1998. The oldest Rex Sole aged from BC was a 34 cm female aged at 15 years, though larger, non-aged individuals have been caught up to length 58 cm. This species has been aged up to 29 years (female) in the Gulf of Alaska [@abookire2006]. Otoliths have been collected on BC surveys from 2000--present although they have not been aged.

Rex Sole are a commonly caught flatfish species in the WCVI (West Coast Vancouver Island) synoptic groundfish bottom trawl surveys (Figure \@ref(fig:rex-surv-set-plot)) and although they are caught in the commercial mixed-species bottom trawl fishery (Figures \@ref(fig:rex-catch-plot) and \@ref(fig:rex-cpue-map)), there is no directed fishery or assigned quota for them.
They are commonly caught with Pacific Cod, Dover Sole, English Sole and Arrowtooth Flounder. Rex Sole have never been formally assessed in BC; however, biomass of Rex Sole in Hecate Strait was estimated in 2012 using research survey data collected between 1984 and 2003 [@fargo2012].
The WCVI synoptic bottom trawl survey shows a moderate decline in biomass density from ~2008 to 2012 and an increase from 2014 to 2018 (Figures \@ref(fig:rex-surv-set-plot) and \@ref(fig:rex-surv-index-plot)).
Standardized commercial trawl CPUE (catch per unit effort) shows an increase in the late 1990s, a moderate decline from ~2004 to 2010, and a moderate increase until about 2016 (Figure \@ref(fig:rex-surv-index-plot)).
There are many samples of length, weight, and maturity of Rex Sole from the WCVI synoptic survey, but no aged fish (Figure \@ref(fig:rex-samples)).
...
In the US, Rex Sole are caught in the bottom trawl fishery and are considered to form three separate stocks: the Gulf of Alaska stock (GOA), the Bering Sea/Aleutian Islands (BSAI) stock and the West Coast stock from Washington to California. The GOA stock is assessed every four years using an age-structured model and was last fully assessed in 2017 [@mcgilliard2017]. The BSAI stock is assessed as part of the "other flatfish complex" of which Rex Sole and Starry Flounder make up most of the commercial harvest. This stock is also assessed every four years, and was last fully assessed in 2015 [@wilderbuer2015]. The West Coast stock was last assessed in 2013 using "extended simple" Stock Synthesis in which a depletion-based stock reduction type analysis is extended to allow for the inclusion of index data (and potentially length and age data) [@cope2015].

What follows is a demonstration of the MP Framework through a case study of the Rex Sole stock on the west coast of Vancouver Island. This case study is *not* meant to be used for formal science advice. We frame our case study according to the steps outlined in Section \@ref(sec:approach).
...
The decision to be made is which MP to use to determine catch limits for the period until the next available catch advice. The time-frame for making the decision would be stated in the request for science advice. In a real application of the MP Framework additional aspects of the decision context would be established through a technical committee. See Section \@ref(sec:approach1).

## STEP 2: SELECTION OF OBJECTIVES AND PERFORMANCE METRICS  {#sec:results-rex2}

We defined the objectives for Rex Sole based on the provisional objectives presented in Section \@ref(sec:approach2). These objectives are guided by the PA Framework of which elements are incorporated into the Fish Stocks provisions of the *Fisheries Act*.
...
We measured the performance metrics as the probability integrating over all years and replicates simultaneously.
...
We defined six OM reference-set scenarios that differed from each other in these respects and two OM robustness-set scenarios (Table \@ref(tab:rex-scen)) with all parameterization described in Appendix \@ref(app:desc-om-rex). We describe each of the scenarios below.
...
Prior to 1996, reported catches for WCVI Rex Sole were much lower than post-1995 and there were no reported discards (Figure \@ref(fig:rex-catch-plot)).
...
The SRA model parameter $C^{\textrm{eq}}$, describes the equilibrium catch in weight prior to the first year of the model (1996 for Rex Sole), which is used to calculate equilibrium fishing mortality prior to the first year of the model $F^{\textrm{eq}}$, which in turn is used to calculate total mortality $Z^{\textrm{eq}}$ needed to calculate recruits and numbers in the first year of the model (Equation \@ref(eq:sra-om-ceqpred)).
...
We attempted to fit $C^{\textrm{eq}}$ values ranging from 0 to 2.5 times the catch in 1996, which imply that equilibrium catch in 1995 was 0% to 250% of catch in 1996.
The SRA model estimated implausibly high values of fishing mortality $F$ ($F > 3$) if $C^{\textrm{eq}} < 200\%$ 1996 catch suggesting that there may be considerable missing discarded catch prior to 1996 and the stock may not have been close to $B_0$ in 1995.
Therefore, our reference set includes only two levels of catch equilibrium that we thought rendered plausible operating model conditioning.
We include an additional scenario that starts closer to $B_0$ but excludes commercial CPUE in the robustness set.

We think this assumption of missing discarded catch prior to 1996 is consistent with the data and what we know about the multispecies groundfish fishery for three reasons.
First, standardized commercial CPUE shows an increasing trend from 1996 to 2003 before the SYN WCVI survey began (Figure \@ref(fig:rex-surv-index-plot)).
While there are many caveats to the commercial CPUE timeseries, two factors lead us to place some faith in the late 1990s trend: (1) commercial CPUE has somewhat mirrored the broad patterns seen in the SYN WCVI survey index since the survey began (Figure \@ref(fig:rex-surv-index-plot)) and (2) Rex Sole have not been a targeted species by the commercial trawl fleet.
This increase in commercial CPUE from 1996 to 2003 with the advent of 100% at-sea monitoring suggests there may be substantial missing discarded catch prior to 100% at-sea-observer coverage in 1996.
Second, the SYN WCVI survey has shown approximately a doubling in its relative index value from the early 2000s to 2019, which would also be consistent with the stock starting in a somewhat depleted state in 1996.
Third, other species that are commonly caught with Rex Sole in catch records since 1996 were reported caught in substantial quantities before 1996 (TODO FINISH).
It therefore seems reasonable to assume that there is considerable missing discarded catch prior to 1996.

In the end, we considered three reference-set scenarios that varied in the assumed value of $C^{\textrm{eq}}$ (and one robustness-set scenario) (Table \@ref(tab:rex-scen)).

* The scenario "Ceq. 200%" assumes that equilibrium catch prior to 1996 was twice that in 1996.

* The scenario "Ceq. 250%" assumes that equilibrium catch prior to 1996 was 2.5 times that in 1996.

* The scenario "No CPUE Ceq. 250%" excludes the commercial CPUE index from the SRA model (Figure \@ref(fig:rex-surv-index-plot)). When the commercial CPUE data were excluded, values less than Ceq. 250% rendered implausible SRA model fits without other changes to the model such as shifting length selectivity ($F > 3$).

#### Steepness

We considered one alternative scenario that differed in terms of the steepness of the stock-recruit relationship (Table \@ref(tab:rex-scen)).
Unless otherwise stated, all other scenarios drew from a baseline beta distribution $h \sim \textrm{beta}(13.4, 2.4)$, which results in a mean and standard deviation of 0.85 and 0.09 (see Section \@ref(app:desc-stock-h-rex)).

* The scenario "Higher steepness" uses the steepness $h$ value of 0.95.

#### Natural Mortality

We considered one alternative scenario that differed in terms of the value of natural mortality (Table \@ref(tab:rex-scen)).
Unless otherwise stated, all other scenarios drew from a baseline uniform distribution *M* = 0.17--0.25 *y*^-1^, which starts at the value of 0.17 *y*^-1^ used for the Gulf of Alaska stock assessment of Rex Sole (@mcgilliard2017, see Section \@ref(app:desc-stock-m-rex)) and explores a slightly higher range since *M* is likely to be somewhat higher at a lower latitude.
...
We considered one alternative scenario that differed in terms of the length and age selectivity of the commercial fleet and the survey (Table \@ref(tab:rex-scen)).
Unless otherwise stated, all other scenarios drew from a length selectivity curve that approximately matched the maturity ogive ($L^5 = 22$, $L^\textrm{FS} = 32$).

* The scenario "Lower selectivity" considers that the selectivity curve could be shifted to the left resulting in both the survey and the commercial fleet capturing younger fish ($L^5 = 17$, $L^\textrm{FS} = 28$).

We considered the above six scenarios to represent the major sources of plausible uncertainty for Rex Sole and grouped them into the "reference set" of OMs (see Section \@ref(sec:best3)).

### The Robustness Set

To illustrate application of the framework in the presence of additional sources of structural uncertainty, we included two robustness scenarios (Table \@ref(tab:rex-scen)):

* The "No CPUE Ceq. 50%" scenario assumes that catch at equilibrium in year 1995 was 50% of the catch in 1996. In other words, this scenario assumes that the stock started in a much less depleted state then the other scenarios. In order to fit such a model with plausible $F$ values, we also had to omit the commercial CPUE and use the selectivity curve described in the "Lower selectivity" scenario.

* The "M increasing" scenario assumes that *M* increases monotonically from 0.2 to 0.4 *y*^-1^ over the projection period. This could represent increasing natural mortality due to a process such as climate change. We include this scenario as an example of including a time-varying parameter in the projection period that reflects an underlying environmental process and to assess robustness of MPs to such a scenario. We do not necessarily expect this particular scenario to be included in applications of the Framework.
...
We included a detailed explanation of the initial OM specification in Appendix \@ref(app:desc-om-rex).

### OM CONDITIONING

We took the initial OMs and conditioned them using the SRA model described in Appendix \@ref(app:app:sra).
We condition of the models using commercial catch from 1996--2019 and the synoptic WCVI (SYN WCVI) trawl relative biomass index and commercial bottom trawl CPUE.
We downweighted the commercial CPUE slightly by inflating the standard errors 1.5 times since we have greater faith in the stratified random survey reflecting underlying biomass trends.
We specified a range of $C^{\textrm{eq}}$ values as described above to reflect uncertainty about initial stock depletion.

The SRA was able to fit to the WCVI synoptic trawl survey relative biomass index and commercial bottom trawl CPUE reasonably well (Figure \@ref(fig:rex-survey-fits)). 
...
The models fit the catch almost perfectly due to the specification of the models with observation errors of only 0.01 standard deviations on the catch (Equation \@ref(eq:sra-om-like-catch)).
...
We used the SRA to populate in the conditioned OMs:
...
The reference and robustness scenarios rendered a range of these parameter values (Figures \@ref(fig:rex-sra-conditioned-parameters), \@ref(fig:rex-F-om), \@ref(fig:rex-recdev-om)).
In fitting the SRA across replicate draws from the initial OMs, any models that did not converge or had $F_{a,y} > 3$ were discarded.
The vast majority of replicates were retained (Table \@ref(tab:rex-sra-fraction)), but by discarding the replicate initial OM parameter values associated with these discarded models, the input parameters carried through to the conditioned OMs varied slightly (Figure \@ref(fig:rex-sra-conditioned-parameters2)).
The discarded parameter values represent implausible parameter combinations according to the SRA model.
The implied spawning stock biomass depletion trajectories during the historical period from the eight OMs are somewhat similar with the exception of the "No CPUE Ceq. 50%" scenario (Figure \@ref(fig:rex-depletion-om)).
...
map_dfr(oms, ~tibble(nsim = .x@nsim), .id = "scenario") %>%
...
(ref:fig-rex-sra-conditioned-parameters) Histograms of parameters estimated as part of the SRA. Fishing mortality at age, the historical depletion trajectory, and historical recruitment deviations are also derived from the SRA model. Apical fishing mortality by year are shown in Figures \@ref(fig:rex-proj-ceq200)--\@ref(fig:rex-proj-no-cpue) and Figures \@ref(fig:rex-proj-inc-m)--\@ref(fig:rex-proj-inc-m), and historical depletion trajectories are shown in Figure \@ref(fig:rex-depletion-om).
D refers to depletion. AC refers to $\theta_\textrm{AC}$.

```{r rex-sra-conditioned-parameters, out.width="0.85\\textwidth", fig.asp=1.4, fig.cap="(ref:fig-rex-sra-conditioned-parameters)"}
...
x <- oms %>% set_names(sc2$scenario_human) %>%
  ...
  reshape2::melt(id.vars = "Scenario") %>% dplyr::filter(!(variable == "R0" & value > 1e7))
...
Frequency polygons for parameters filtered as part of the SRA. These are effectively polygons outlining a histogram. These distributions are sampled stochastically as input into the SRA and any samples associated with non-converged SRA models are discarded. Since most SRA models converged, these distributions are similar across scenarios unless they are specified differently. The large spikes represent fixed parameter values in the OM. sigma_R refers to $\sigma_R$.
...
Early explorations suggested little effect of the three harvest control rules described in Appendix \@ref(app:MPs) (presumably because the simulation spent little time on the "ramp") a little difference between the Fox and Schaefer production models.
Therefore, with the SP models we focused mostly on tuning the prior on $r$, the intrinsic population growth rate.
We set evaluated three priors on $r$: $\textrm{Normal}(0.4, 0.1)$, $\textrm{Normal}(0.5, 0.1)$, and $\textrm{Normal}(0.6, 0.1)$.
With the Schaefer surplus production function, these correspond to $F_\textrm{MSY} = 0.2$, $F_\textrm{MSY} = 0.25$, and $F_\textrm{MSY} = 0.3$, which roughly mimic the values of natural mortality $M$ explored in the operating models. 
...
csasdown::csas_table(mp, caption = "Candidate MPs. The `.` in front of some MPs denotes versions of the MPs that only observes the survey index every second year to mimic our biennial WCVI synoptic survey.", col_names = c("Management procedure", "MP type"))
```
...
We ran the closed-loop simulations across 250 stochastic replicates using DLMtool version 5.4.2, `r rversion`, and the simulation random seed set to `r oms[[1]]@seed` with the OMs and MPs described above.
...
We deemed the number of replicates sufficient since the rank order of MPs was remaining consistent by 250 replicates (Figure \@ref(fig:rex-converge)).

We began by assessing average and minimum performance across all candidate MPs for the reference set of OMs (Figures \@ref(fig:rex-tigure-avg) and \@ref(fig:rex-tigure-min)) to determine which MPs would be carried forward as satisficed MPs [@miller2010].
...
We could apply these criteria to either the average (Figures \@ref(fig:rex-tigure-avg)) or minimum performance (Figures \@ref(fig:rex-tigure-min)) and such decision is likely decision to be made by the technical working group or decision-makers coupled with stakeholders in an applied setting.
In this case study, it makes little difference whether we choose the average or minimum performance metrics to apply the satisficing criteria.
For this case study we choose to apply the site is facing criteria to the minimum performance metrics.
This implies that the satisficed MPs perform at least according to these criteria even in the worst case reference-set scenario.
Applying these criteria left us with `r number_to_word(length(mp_sat_b4_cut))` MPs (`r gfutilities::commify(sort(mp_sat_b4_cut))`).

We additionally decided to omit four MPs from further consideration.
First, to we chose to retain only one satisficed surplus-production MP (.SP8040_0.6) since the surplus-production MPs that differed only in production function shape (.SP6040_0.6_fox) or harvest control rule (.SP6040_0.6 and .SP4010_0.6) had near identical performance (Figure \@ref(fig:rex-tigure-min)).
Second, we removed CC_hist20 since it was "dominated" by the other constant catch MPs. I.e., LT LRP and STC were both exceeded by all other constant-catch MPs (Figure \@ref(fig:rex-tigure-min)).
This left us with `r number_to_word(length(mp_sat))` MPs (`r gfutilities::commify(sort(mp_sat))`).

(ref:fig-rex-converge)
Assessing convergence of the Rex Sole closed-loop simulations on a consistent rank order of MPs within performance metrics.
Colours represent individual satisficed and reference MPs.
Lines that are no longer crossing by the final replicates indicate simulations that are consistent with convergence.
Although not shown, we also checked that the satisficing rules had converged (the selection of satisficed MPs would not change with further replicates).
...
The colour shading reflects the underlying numbers.
Outlined cells represent MPs that met a particular performance metric satisficing criteria.
...
We can see some variability in performance within the individual reference-set scenarios with the most variability in the longer term (or entire projection) performance metrics: LTY USR, FMSY, LTC, and AADC (Figure \@ref(fig:rex-tigure-panel)).
For the two performance metrics we used in the initial satisficing step, LT LRP varied between `r round2(min(pm_avg_sat$"LT LRP"))` and `r round2(max(pm_avg_sat$"LT LRP"))` and STC varied between `r round2(min(pm_avg_sat$STC))` and `r round2(max(pm_avg_sat$STC))`.

A dot-and-line plot of the performance metrics aggregated across scenarios is perhaps most instructive at comparing performance across satisficed MPs (Figure \@ref(fig:rex-dot-lines)).
Long-term catch (LTC) is roughly comparable between a fishing strategy of fishing at *F*/*F*~MSY~ and the satisficed Itarget or .SP8040_0.6 MPs (Figure \@ref(fig:rex-dot-lines)).
Short-term catch (STC) of all satisficed MPs exceeds these reference MPs (Figure \@ref(fig:rex-dot-lines)).
Fixed catch MPs of 1.1 or 1.2 times average catch from the last five years renders similar performance to other satisficed MPs across most performance metrics within the reference-set scenarios, with somewhat higher LTC and AADC metrics (Figure \@ref(fig:rex-dot-lines)).
However, these fixed-catch MPs do not incorporate feedback between the management and biological system to react to unexpected circumstances.

There is not a large degree of trade-off between performance metrics amongst most of these satisficed MPs within the reference-set scenarios (Figures \@ref(fig:rex-tradeoff-reference) and \@ref(fig:rex-spider-satisficed-mps-avg)).
Within the "Ceq. 200%" scenario, there is a slight trade-off between STC and LT LRP (Figure \@ref(fig:rex-tradeoff-reference)).

<!--
Figure \@ref(fig:rex-dot-lines)
Figure \@ref(fig:rex-tradeoff-reference)
Figure \@ref(fig:rex-spider-satisficed-mps-avg)
Figure \@ref(fig:rex-proj-index)
Figures \@ref(fig:rex-proj-ceq200)--\@ref(fig:rex-proj-no-cpue)

Figure \@ref(fig:rex-kobe)
Figure \@ref(fig:rex-worm)
...
We can look at timeseries trajectories of the projected survey index (Figure \@ref(fig:rex-proj-index), *B*/*B*~MSY~, *F*/*F*~MSY~, and catch to further understand performance across the various MPs and reference set OMs (Figures \@ref(fig:rex-proj-ceq200)--\@ref(fig:rex-proj-no-cpue)).
...
Kobe plots demonstrate either the final *B*/*B*~MSY~ vs. *F*/*F*~MSY~ status of the various replicates (Figure \@ref(fig:rex-kobe)) or the trajectory of these status values through time (Figure \@ref(fig:rex-worm)).
...
These are the same data underlying Figure \@ref(fig:rex-tigure-avg) and Figure \@ref(fig:rex-tigure-min) but shown for individual reference-set scenarios and only for satisficed MPs.
...
```{r rex-tradeoff-reference, fig.cap="Trade-off between LT LRP and STC performance metrics across the reference-set scenarios.", out.width="\\textwidth"}
knitr::include_graphics(here("report/figure/rex-tradeoff-refset.png"))
```

\clearpage

```{r rex-spider-satisficed-mps-avg, fig.cap="Radar-plot representation of average performance metric trade-offs for the reference-set scenarios. The outside of the hexagon represents a performance metric probability of 1.0 in the middle represents a value of 0. Dashed lines represent reference MPs.", out.width="5in"}
...
The "No CPUE Ceq. 50%" robustness scenario represents a considerably different biomass and fishing mortality historical trajectories to the reference set.
This scenario considers the stock to be considerably smaller (smaller $R_0$ values in Figure \@ref(fig:rex-sra-conditioned-parameters)) with high $F$ values around 2005 (Figure \@ref(fig:rex-F-om)).
In this robustness scenario, the Itarget MPs are able to maintain LT LRP $>$ 0.60 whereas the constant catch and surplus production model achieve LT LRP $<$ 0.35 (Figures \@ref(fig:rex-tigure-panel-rob) and \@ref(fig:rex-dots-satisficed-mps-robust)).
Other performance metrics show less contrast across the MPs, although LT USR also remains higher for the Itarget vs. constant-catch or surplus-production MPs (Figure \@ref(fig:rex-tigure-panel-rob) and \@ref(fig:rex-dots-satisficed-mps-robust)).

The scenario demonstrates a stronger trade-off between the performance metrics then the reference set scenarios did (Figures \@ref(fig:rex-tradeoff-robust) and \@ref(fig:rex-spider-satisficed-mps-robust)).
In particular, we can see a trade-off between STC and LT LRP (Figures \@ref(fig:rex-tradeoff-robust)).
The constant catch MPs achieve the highest STC values but at the expense of lower LT LRP values.
The Itarget MPs somewhat strike a balance between the two (Figures \@ref(fig:rex-tradeoff-robust)).
The projection figures demonstrate the data underlying the performance metrics (Figure \@ref(fig:rex-proj-no-cpue-light)).
Within this robustness scenario, over time, the .SP8040_0.6, CC1.1, and CC1.2 MPs all have a tendency to overfish the stock across replicates (Figure \@ref(fig:rex-proj-no-cpue-light)).

The satisficed MPs were relatively robust to linear increases in natural mortality in the projection period with the exception that all satisficed MPs had a considerably lower probability of achieving long-term catch (LTC) performance metrics (Figures \@ref(fig:rex-tigure-panel-rob)--\@ref(fig:rex-spider-satisficed-mps-robust)).
Notably, the Itarget satisficed MPs maintained LT LRP $>$ 0.9 *despite* the increasing *M* whereas the fixed-catch and surplus-production MPs all fell below 0.9.
Looking at the projections, we can see that these MPs were able to ramp down catch appropriately as *M* increased (Figure \@ref(fig:rex-proj-inc-m)) to maintain most replicates above the LRP.
...
By plotting the projections from all scenarios (Figures \@ref(fig:rex-proj-ceq200)--\@ref(fig:rex-proj-no-cpue) and \@ref(fig:rex-proj-no-cpue-light)--\@ref(fig:rex-proj-inc-m)) on the same panels, we can examine the sensitivity of the projections to the various scenario conditions (Figure \@ref(fig:rex-proj-scenarios)).
The historical trajectory is most sensitive to the assumptions of the "No CPUE Ceq. 50%" scenario compared to the others.
The projected trajectory is relatively similar across the reference-set scenarios, while differing markedly for the "No CPUE Ceq. 50%" and "M increasing" robustness scenarios.
