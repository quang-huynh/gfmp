...
In addition, to illustrate application of the framework in the presence of additional sources of structural uncertainty, we included two robustness scenarios (Table \@ref(tab:rex-scen)).

* The "No CPUE Ceq. 50%" scenario assumes that catch at equilibrium in year 1995 was 50% of the catch in 1996. In order to fit such a model with plausible $F$ values, we also had to omit the commercial CPUE and use the selectivity curve described in the "Lower selectivity" scenario.
...
Fits to the historical indices of abundance from the `r number_to_word(nrow(sc))` OMs are shown in Figure (\@ref(fig:rex-survey-fits)).
Estimates of depletion (*B*/*B*~0~) during the historical period from the nine OMs are shown in Figure (\@ref(fig:rex-depletion-om)).

We used the SRA to populate in the OM:
...
- $\theta_\textrm{AC}$ (first-order autocorrelation of recruitment deviations)
- $B_{t_c}/B_0$ (depletion in the last historical year)

```{r rex-om-load, results='asis'}
sp <- "rex"
...
```{r rex-sra-conditioned-parameters2, out.width="\\textwidth", fig.asp=0.85, fig.width=7, fig.cap="fig-rex-sra-conditioned-parameters2"}
...
Any replicates that did not converge or in which $F_y$ reached 3 were discarded and the corresponding draw of other OM parameters (growth parameters, length-weight parameters, maturity parameters, selectivity parameters, stock-recruit parameters) were discarded.
The vast majority of replicates converged and were retained: TODO
...
We ran the closed-loop simulations across 250 stochastic iterations using DLMtool version 5.4.2 with the OMs and MPs described above.
...
Lines that are no longer crossing by the final iterations indicate simulations that are consistent with convergence.
We also checked that the satisficing rules had converged (the selection of satisficed MPs would not change with further iterations).
...
We can also look at "Kobe" plots that demonstrate either the final B/B~MSY~ vs. F/F~MSY~ status of the various simulations or the trajectory of these status values through time (Figure \@ref(fig:rex-kobe) and \@ref(fig:rex-worm)).
...
```{r rex-proj-index, fig.cap="Historical and projected SYN WCVI relative biomass index values. Vertical dashed line represents 2019. The shaded region represents the 95\\% quantile and individual lines represent four sample iterations.", out.width="\\textwidth"}
