...
In the US, Rex Sole are caught in the bottom trawl fishery and are considered to form three separate stocks: the Gulf of Alaska (GOA) stock  [@mcgilliard2017], the Bering Sea/Aleutian Islands (BSAI) stock [@wilderbuer2015], and the West Coast (WC) stock from Washington to California [@cope2015]. 
...
This is likely partly due to a lack of at-sea observer coverage and reporting requirements prior to 1996.
...
We include an additional scenario with the 1995 stock closer to $B_0$ in the robustness set (Section \@ref(sec:results-robustness-set)). 
...
Increasing CPUE is also possibly evidence of a stock rebuilding from a depleted state.
While there are many caveats associated with commercial CPUE timeseries, two factors suggest this trend is plausible: (1) commercial CPUE has somewhat mirrored the broad patterns seen in the SYN WCVI survey index since the survey began (Figure \@ref(fig:rex-surv-index-plot)); and (2) Rex Sole have not been a targeted species by the commercial trawl fleet.
In the absence of targeting, CPUE trends may more closely match abundance.
...
Third, other species that are commonly caught with Rex Sole in catch records since 1996 were reported caught in substantial quantities before 1996 (**TODO FINISH ... will need to provide a table or figure for this**).
It therefore seems reasonable to assume that there is considerable unrecorded catch prior to 1996.

We considered three reference-set scenarios and one robustness-set scenario that varied in the assumed value of $C^{\textrm{eq}}$ (Table \@ref(tab:rex-scen)). The three reference-set scenarios are:
...
* The scenario "Higher steepness" uses the value $h = 0.95$.
...
We considered one alternative scenario that differed in terms of the length- and age- selectivity of the commercial fleet and the WCVI Synoptic survey (Table \@ref(tab:rex-scen)).
...
* The "No CPUE Ceq. 50%" scenario assumes that catch at equilibrium in year 1995 was 50% of the catch in 1996 ($C^{\textrm{eq}} = 0.5$). In other words, this scenario assumes that the stock started in a much less depleted state than the other scenarios. In order to fit the SRA model with plausible $F_{a,y}$ values, we also had to omit the commercial CPUE and use the selectivity curve described in the "Lower selectivity" scenario.

* The "M increasing" scenario assumes that $M$ increases monotonically from 0.2 to 0.4 *y*^-1^ over the projection period. This could represent increasing natural mortality due to a process such as climate change or shifts in predator abundance. We include this scenario simply to illustrate including a time-varying parameter in the OM projection period that reflects an underlying environmental process, and to assess robustness of MPs to such a scenario.
...
After specifying most of the OM parameters (Appendix \@ref(app:desc-om-rex)), we conditioned the OMs using the SRA model described in Appendix \@ref(app:app:sra).
...
The reference and robustness scenarios rendered a range of estimated parameter values (Figures \@ref(fig:rex-sra-conditioned-parameters), \@ref(fig:rex-F-om), \@ref(fig:rex-recdev-om)).
...
There was variability in performance among the individual reference-set scenarios with the most variability in both the long-term and entire projection period performance metrics: LT USR, FMSY, LTC, and AADC (Figure \@ref(fig:rex-tigure-panel)).
...
Trade-offs among performance metrics for most of the satisficed MPs were relatively minor within the reference-set scenarios (Figures \@ref(fig:rex-tradeoff-reference) and \@ref(fig:rex-spider-satisficed-mps-avg)).
Within the "Ceq. 200%" scenario, there was a slight trade-off between STC and LT LRP (Figure \@ref(fig:rex-tradeoff-reference)).
...
This scenario estimates the stock to be smaller (smaller $R_0$ values in Figure \@ref(fig:rex-sra-conditioned-parameters)) with high $F_t$ values around 2005 (Figure \@ref(fig:rex-F-om)).
In this scenario, the Itarget MPs are able to achieve LT LRP $>$ 0.60, while the constant catch and surplus production model achieve LT LRP $<$ 0.35 (Figures \@ref(fig:rex-tigure-panel-rob) and \@ref(fig:rex-dots-satisficed-mps-robust)).
...
This scenario demonstrates stronger trade-offs among performance metrics than the reference set scenarios (Figures \@ref(fig:rex-tradeoff-robust) and \@ref(fig:rex-spider-satisficed-mps-robust)).
...
Within this scenario, over time, the .SP8040_0.6, CC1.1, and CC1.2 MPs all have a tendency to overfish the stock (Figure \@ref(fig:rex-proj-no-cpue-light)).
...
By looking at the projections, it is possible to see that the Itarget MPs were able to ramp down catch appropriately as $M$ increased (Figure \@ref(fig:rex-proj-inc-m)) to maintain most replicates above the LRP.
...
By plotting the projections from all scenarios (Figures \@ref(fig:rex-proj-ceq200)--\@ref(fig:rex-proj-no-cpue) and \@ref(fig:rex-proj-no-cpue-light)--\@ref(fig:rex-proj-inc-m)) on the same panels, we can examine the sensitivity of the projections to the various OM scenario conditions (Figure \@ref(fig:rex-proj-scenarios)).
