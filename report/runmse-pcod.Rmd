...
real_catchpc <- mutate(real_catch, synthetic = ifelse(Year >= 1950 & Year <= 2018, Observed.catch, Predicted.catch)) #Don't use predicted catch for P cod

all_years <- data.frame(Year = (2018 - fleetpc@nyears):2018)
real_catch <- dplyr::left_join(all_years, real_catch)

set.seed(1938)
catch <- pcod_historical@TSdata$Catch %>%
  reshape2::melt() %>%
