...
oldparpc <- par()
plotStockpc(ompc, nsamp = 3, incVB = TRUE)
```
...
```{r, eval=TRUE, fig.asp=1}
plotFleetpc(fleetpc, Stock = stockpc)
```
...
ompc@nsim <- 30
short_historicalpc <- runMSE(ompc, Hist = TRUE, parallel = FALSE)
ompc@nsim <- 250L

# names(short_historical@TSdata)
# dim(short_historical@TSdata$Catch)

real_catchpc <- read.csv(here::here("report/data/shortraker-corrected-catch.csv"))
real_catchpc <- mutate(real_catch, synthetic = ifelse(Year >= 1990 & Year <= 1995, Predicted.catch, Observed.catch))
