...
MP Frameworks differ from conventional assessments in two key ways: (1) Reference points and stock status are not explicitly reported; and (2) Objectives related to the probability of breaching reference points must be agreed at the beginning of the process, i.e., at Step 2 of the best practices (Section \@ref(sec:best2)).
...
In established MSE processes [e.g., @cox2008a], informal evaluation evaluation of the performance of MPs may be done at regular intervals as the MP is applied and new data are gathered (e.g., survey and commercial CPUE information).
...
<!-- TODO: SA please have a look at this. It gets a bit wishy washy at the end but I think we need to say something like that. You might have some better statistical language-->

The MP Framework provides the tools to test the "value of information" (VoI) on the performance of MPs. 
VoI analyses assess whether performance can be improved with the addition of  resources invested in data-collection. 
Or, conversely, to evaluate whether the same performance could be achieved if fewer data were available. 
These are essentially sensitivity analyses to addition or removal of data sources.

Typical data types that could be included in a VoI analysis are age-composition data, survey data and survey CV.
Sensitivity of MP performance to changes in survey frequency or CV (representing increases or decreases in survey coverage) could easily be tested using the framework.
The framework could also be used to evaluate whether improved MP performance could be achieved by ageing the large archive of otoliths for Rex Sole.
...
DLMtool's companion package MSEtool contains a large number of data-rich MPs that could be tested [huynh_msetool_2019].
This type of analysis could be used to answer questions about whether data rich MPs such as full age-structured models would better meet management objectives for a given fish stock and, if so, how much ageing data would be required for a full assessment.
