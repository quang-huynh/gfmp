...
The following are OM "slots" that we suggest by default are "turned off" or set to default values for the sake of simplicity. On a case-by-case basis an analysis might choose to set some of these lots to other values or include other values as part of a robustness set. However, unless specified otherwise, the following slots can be assumed to be specified as follows.
...
By default we will assume that natural mortality is not time-varying, although this might be an important slot to explore as part of a robustness that for some stocks.
...
We will assume that fishing mortality is not time varying.
