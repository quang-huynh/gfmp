...
In addition to the data-based MPs, we suggest a surplus production model be considered amongst candidate MPs.
We provisionally include the surplus production model coded in MSEtool [@huynh_msetool_2019] and based on @fletcher1978 (Section \@ref(sec:mp-sp)) paired with a number of possible harvest control rules (HCRs). We suggest inclusion of both a @schaefer1954 and @fox1970 production curve since it is not clear, until simulation tested, which will generate better performance statistics for a given stock.
We suggest a weakly informative prior be set on the intrinsic greater population increase $r$ based on TODO. Alternative priors may be considered in alternative OMs.
...
Data-limited MP development is a rich area of research and is probably in its infancy.
...
While we cannot predict what those new MPs will be, provided they are easy to communicate and can be easily applied to both simulated and real data, we can rest assured that new MPs will be rigorously tested through the closed-loop simulation process that is inherent to this MP Framework before they are adopted.
