...
We used the DLMtool software [@carruthers2018] to develop OMs for Shortraker Rockfish and Rex Sole. DLMtool OMs are organised into four main components representing a real fished system: 1. population dynamics of the fish stock (e.g., growth, recruitment, mortality); 2. fishery dynamics (e.g., selectivity, spatial targeting); 3. observation processes (e.g., bias and precision in survey indices); and 4. management implementation (e.g., percentage overages of catch limits). Parameters ("slots") representing processes under the four components are described in detail in Section \@ref(app:dlmtool-om).

DLMtool allows the incorporation of uncertainty in most parameters of the OM through specification of a distribution, typically a uniform distribution between two bounds. Additional characterization of uncertainty is achieved by developing alternative OMs to isolate the effects of specific sources of uncertainty on performance of MPs.

We developed **XX** OMs for each stock (Shortraker Rockfish and Rex Sole), dividing the OMs into a reference set, with OMs characterizing key model uncertainties, and a robustness set, to explore the effects of other possible structural uncertainties [@rademeyer2007].
