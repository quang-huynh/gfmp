...
DLMtool OMs are organised into four main components representing a real fished system: 1. population dynamics of the fish stock (e.g., growth, recruitment, mortality); 2. fishery dynamics (e.g., selectivity, spatial targeting); 3. observation processes (e.g., bias and precision in survey indices); and 4. management implementation (e.g., percentage overages of catch limits). Parameters ("slots") representing processes under the four components are described in detail in Section \@ref(app:dlmtool-om) and in Appendix B of @carruthers2018.

DLMtool allows the incorporation of uncertainty in most OM parameters through specification of a distribution, typically a uniform distribution between two bounds.See Appendix B of @carruthers2018 for a full list of parameters for which a distribution is specified. To isolate the effects of specific sources of uncertainty on performance of MPs, we recommend development of alternative OMs that change the value (or distribution) of one parameter or data source of interest.

We recommend developing more than one OM, dividing OMs into a reference set representing key plausible model uncertainties thought to have a large impact on results, and a robustness set for testing sensitivity to a broader range of structural uncertainties [@rademeyer2007].  
...
To ensure reproducible research and transparency, we suggest that the full specification of all OM parameters be clearly documented in appendices attached to the working paper. 

We note that DLMtool includes a very large number of parameters in the OM that can vary through time, or which can be set to be deliberately biased. To simplify the operating model and focus on what are likely the most important axes of uncertainty, we suggest fixing a number of parameters to be time-invariant and unbiased (Appendix \@ref(app:default-slots)). Exceptions may occur when one or more of these parameters represent axes of uncertainty that should be investigated in the robustness set for specific stocks.

Best practice recommends calibrating OMs to historical observations (especially indices of abundance) to improve plausibility. Of course, there are multiple ways to achieve goodness of fit to observed data, and a close fit to observations does not mean that an OM is correct. This is one of the main reasons for constructing multiple OMs, to explore the impacts of alternative model assumptions on performance of MPs.

DLMtool's companion software package, MSEtool [@huynh_msetool_2019], includes an implementation of a stock reduction analysis (SRA) [@kimura1982; @walters2006], which is effectively a catch-at-age model that estimates the combinations of historical fishing mortality and recruitment that would be consistent with the observed data. The SRA implementation in MSEtool can be conditioned on catch or effort time series. For most applications for BC groundfish stocks, we suggest conditioning on catch. Historical trajectories of catch tend to be more reliable than time series of effort, especially given uncertainties in how to best represent and interpret effort in multispecies fisheries. 

An advantage of using this approach for calibrating OMs is it allows fitting to multiple indices of abundance. The model can also accommodate multiple fishing fleets
...
We used DLMtool and MSEtool [@carruthers2018] to develop and calibrate OMs for two BC groundfish species, Rex Sole and Shortraker Rockfish. The reference set of OMs are described in detail in Appendices \@ref(app:desc-om-sr) and \@ref(app:desc-om-rex) for Rex Sole and Shortraker Rockfish, respectively. 
...
Here, we describe the types of MPs available, describe the process by which we identified a provisional set of MPs, and explain how some MPs were tailored to the BC groundfish needs.
...
For the framework, we focus on MPs that make TAC recommendations based on how groundfish are managed in BC.
...
Data-based MPs take data generated by an operating model, such as a simulated survey index, apply an algorithm, and make a catch recommendation.
For example, one implementation of the "Iratio" MP [@ices2012; @jardim2015], the mean survey index value from the last two years is divided by the mean survey index value from the years three to five years before present.
This ratio is then multiplied by the previous year's catch to generate a catch recommendation.
If the survey index has been trending up, then the catch recommendation will increase and vice versa. Model-based MPs, on the other hand, fit some statistical population model and apply some harvest control rule to turn the parameter estimates into a catch recommendation.

### Data-based MPs

Data-based MPs can make use a variety of data types including catch, fish lengths, fish ages, and population indexes.
Catches are available for all BC groundfish stocks with relative certainty since 1997 for BC trawl fisheries and 2008 for BC hook and line fisheries and so are a viable data type.
Fisheries independent trawl and longline surveys have been conducted systematically since the early 2000s for BC groundfish and likely represent some of the most informative data for many data-limited groundfish stocks in BC.
Fish lengths are collected on both surveys and commercial fishing trips for many species.
However, length-based MPs often require strong assumptions (e.g., steepness... REF) and require that the simulated length-composition data are sufficiently "messy" to reflect the real-world length-composition data---a tricky assumption to meet (REF) and one we have not sufficiently investigated yet.
Reliable and abundance fish ages are generally not available for the species for which this framework is designed.
For the above reasons, we propose that the MPs that make use of catch and population index data are best suited.

We can divide MPs that make use of population index data into four categories: constant catch, index ratio, index slope, and index target.
(1) Constant-catch MPs set the recommended catch to some fixed level typically based on recent or historical catches.
Importantly, these constant-catch do not incorporate feedback from the population---they make the same catch recommendation regardless of what happens to the stock.
(2) Index-ratio MPs base their catch recommendation on a ratio of a population index in some time period compared to another time period.
(3) Index-slope MPs fit some sort of a regression to population index data and make a catch recommendation based on the slope of that regression.
They are closely related to index-ratio MPs.
(4) Index-target MPs compare recent levels of a population index to a population index at some fixed historical time period to make a catch recommendation.
They differ subtly but importantly from the index-ratio and index-slope MPs as categorized above:
index-ratio and index-slope MPs compare recent index values to a moving window of index values whereas index-target MPs try to maintain the population index at some fixed historical value.

We tailored many of the available data-based MPs to suit the strengths and weaknesses of the BC groundfish data and the typical historical knowledge (Appendix \@ref(app:MPs)).
For example, most of the available survey data for BC groundfish is collected biennially in any one location.
Therefore, we modified many of the index-based MPs to more appropriately handle biennial data. 
Typically, this involved adding variants of MPs that considered longer time windows when calculating averages or slopes to account for half the available data as one would have with an annual survey.
...
In addition to the data-based MPs, we suggest a surplus production (SP) model-based MP be considered amongst candidate MPs.
We provisionally include the SP model coded in the MSEtool R package (REF) and based on Fletcher (1978)(REF) (Section \@ref(sec:mp-sp)) paired with a number of possible harvest control rules (HCRs). We suggest inclusion of both a Schaefer (REF) and Fox (REF) production curve since it is not clear until simulation tested which will generate better performance statistics for a given operating model.
We suggest a weakly informative prior be set on TODO based on TODO and alternative priors may be represented as alternative MPs.
...
1. Build and test operating models for areas associated with a single index. If these areas are considered simultaneously in a single application of the MP Framework then we suggest comparing performance of the MPs across all areas and, if possible, choosing an MP that performs reasonably well across all areas perhaps via a minimax-style solution (REF) choosing the MP that performs the least bad across all areas. If MP performance across areas differ substantially then different MPs may need to be considered for different areas.

2. Develop a single index by "stitching" multiple survey indices together, likely with the application of geostatistical spatiotemporal modeling. This is an active area of research (e.g., REFs) and will likely become more common within the fisheries literature and in stock assessments. While MPs could be developed that average or in some other way simple way combine multiple survey indices (e.g., yelloweye outside REF), geostatistical modelling is likely to offer a more coherent way of combining survey data from multiple protocols or spatial areas information.
...
* Maintaining the status quo TAC

The purpose of reference MPs is not to explore viable management strategies but to bound the range of expected or possible performance (Punt REF). For example, the "no fishing" reference MPs provides information on maximum possible stock levels and the maximum possible rate of rebuilding under a rebuilding scenario. The "fishing at 0.75 F/F~MSY~" MP illustrates performance under an omniscient manager with perfect information. The status quo MP is included not because we think it is necessarily a good management strategy but illustrates what is likely the default had the framework not been implemented and illustrates the long-term performance expectations given current exploitation levels.

### Including new MPs

The candidate MPs proposed here are a provisional library from which to build.
Data-limited MP development is a rich area of research and is probably just in its infancy.
Beyond some adjustments to existing MPs, MP development is not the focus of this report
More MPs may be developed as part of the application of this framework and are likely to be developed elsewhere in the literature and could be included in future applications of the framework.
And, while, we cannot predict what those MPs will be, provided they are easy to communicate and can be easily apply to both simulated and real data, we can rest assured that MPs will be rigorously tested through the closed-loop simulation process that is inherent to this MP Framework.
